...
function extractSubagentPathsFromPackageRoot(packageRoot: string): PackageSubagentPaths {
	...
	const piSubagents = (pkg as { "pi-agents-flow"?: unknown })["pi-agents-flow"];
...
function parseBuiltinOverrideEntry(
	...
	if ("visibility" in input) {
		if (input.visibility === "default" || input.visibility === "hidden" || input.visibility === false) {
			override.visibility = input.visibility;
		} else {
			throw new Error(`Builtin override '${name}' in '${filePath}' has invalid 'visibility'; expected 'default', 'hidden', or false.`);
		}
	}

	if ("invocation" in input) {
		if (input.invocation === "both" || input.invocation === "model" || input.invocation === "user" || input.invocation === "disabled" || input.invocation === false) {
			override.invocation = input.invocation;
		} else {
			throw new Error(`Builtin override '${name}' in '${filePath}' has invalid 'invocation'; expected 'both', 'model', 'user', 'disabled', or false.`);
		}
	}
...
function applyBuiltinOverride(
	...
	if (override.visibility !== undefined) next.visibility = override.visibility === false ? undefined : override.visibility;
	if (override.invocation !== undefined) next.invocation = override.invocation === false ? undefined : override.invocation;
...
function applyCustomAgentOverride(
	...
	if (override.visibility !== undefined) {
		fill("visibility", ["visibility"], override.visibility === false ? undefined : override.visibility);
	}
	if (override.invocation !== undefined) {
		fill("invocation", ["invocation"], override.invocation === false ? undefined : override.invocation);
	}
...
export function buildBuiltinOverrideConfig(
	...
	draft: Pick<AgentConfig, "model" | "fallbackModels" | "thinking" | "systemPromptMode" | "inheritProjectContext" | "inheritSkills" | "defaultContext" | "acceptanceRole" | "disabled" | "visibility" | "invocation" | "systemPrompt" | "skills" | "tools" | "mcpDirectTools" | "extensions" | "subagentOnlyExtensions" | "completionGuard" | "toolBudget"> & Partial<Pick<AgentConfig, "description">>,
): BuiltinAgentOverrideConfig | undefined {
	const override: BuiltinAgentOverrideConfig = {};
	...
	if (draft.visibility !== base.visibility) override.visibility = draft.visibility ?? false;
	if (draft.invocation !== base.invocation) override.invocation = draft.invocation ?? false;
...
export function loadAgentsFromDir(dir: string, source: AgentSource): AgentConfig[] {
	...
	for (const filePath of listFilesRecursive(dir, (fileName) => fileName.endsWith(".md") && !fileName.endsWith(".chain.md"))) {
		...
		let visibility: AgentVisibility | undefined;
		if (frontmatter.visibility !== undefined && frontmatter.visibility.trim()) {
			if (frontmatter.visibility === "default" || frontmatter.visibility === "hidden") visibility = frontmatter.visibility;
			else throw new Error(`Agent '${localName}' has invalid visibility frontmatter; expected 'default' or 'hidden'.`);
		}
		let invocation: AgentInvocation | undefined;
		if (frontmatter.invocation !== undefined && frontmatter.invocation.trim()) {
			if (frontmatter.invocation === "both" || frontmatter.invocation === "model" || frontmatter.invocation === "user" || frontmatter.invocation === "disabled") invocation = frontmatter.invocation;
			else throw new Error(`Agent '${localName}' has invalid invocation frontmatter; expected 'both', 'model', 'user', or 'disabled'.`);
		}
