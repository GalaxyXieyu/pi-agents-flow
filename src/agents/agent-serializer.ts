...
export function serializeAgent(config: AgentConfig, options: SerializeAgentOptions = {}): string {
	...
	if (config.visibility || preserve("visibility")) lines.push(`visibility: ${config.visibility ?? ""}`);
	if (config.invocation || preserve("invocation")) lines.push(`invocation: ${config.invocation ?? ""}`);
