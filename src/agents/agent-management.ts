...
function chainAllowsModelInvocation(chain: ChainConfig, agents: AgentConfig[], capabilityCeiling: ReturnType<typeof resolveCurrentSubagentCapabilityCeiling>): boolean {
	return chain.steps.every((step) => {
		const names: string[] = [];
		if (step.agent) names.push(step.agent);
		if (Array.isArray(step.parallel)) names.push(...step.parallel.map((task) => task.agent));
		else if (step.parallel?.agent) names.push(step.parallel.agent);
		return names.every((name) => {
			const resolved = resolveAgentName(name, agents);
			return Boolean(resolved.agent
				&& canInvokeAgent(resolved.agent, "model")
				&& isAgentAllowedByCapabilityCeiling(resolved.agent.name, capabilityCeiling));
		});
	});
}
...
export function preservedAgentFrontmatterFields(agent: AgentConfig, cfg: Record<string, unknown>): Set<string> {
	...
	if (hasKey(cfg, "visibility")) changed("visibility");
	if (hasKey(cfg, "invocation")) changed("invocation");
...
function applyAgentConfig(target: AgentConfig, cfg: Record<string, unknown>): string | undefined {
	...
	if (hasKey(cfg, "visibility")) {
		if (cfg.visibility === false || cfg.visibility === "") target.visibility = undefined;
		else if (cfg.visibility === "default" || cfg.visibility === "hidden") target.visibility = cfg.visibility;
		else return "config.visibility must be 'default', 'hidden', or false when provided.";
	}
	if (hasKey(cfg, "invocation")) {
		if (cfg.invocation === false || cfg.invocation === "") target.invocation = undefined;
		else if (cfg.invocation === "both" || cfg.invocation === "model" || cfg.invocation === "user" || cfg.invocation === "disabled") target.invocation = cfg.invocation;
		else return "config.invocation must be 'both', 'model', 'user', 'disabled', or false when provided.";
	}
...
function formatAgentDetail(agent: AgentConfig): string {
	...
	lines.push(`Visibility: ${agent.visibility ?? "default"}`);
	lines.push(`Invocation: ${effectiveAgentInvocation(agent)}`);
...
export function handleList(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	const modelVisibleAgents = scopedAgents.filter(isAgentVisibleToModel);
	const agents = modelVisibleAgents.filter((a) => canInvokeAgent(a, "model") && isAgentAllowedByCapabilityCeiling(a.name, capabilityCeiling));
	const restrictedAgents = modelVisibleAgents.filter((a) => !canInvokeAgent(a, "model") || !isAgentAllowedByCapabilityCeiling(a.name, capabilityCeiling));
	const restrictedSources = capabilityCeilingAgentRestrictionSources(capabilityCeiling);
	const chains = d.chains
		.filter((c) => scope === "both" || c.source === "package" || c.source === scope)
		.filter((chain) => chainAllowsModelInvocation(chain, scopedAgents, capabilityCeiling))
		.sort((a, b) => a.name.localeCompare(b.name));
	...
	const lines = [
		"Executable agents:",
		...(agents.length
			? agents.map((a) => `- ${a.name} (${a.source}${a.defaultContext ? `, context: ${a.defaultContext}` : ""}${effectiveAgentInvocation(a) !== "both" ? `, invocation: ${effectiveAgentInvocation(a)}` : ""}${a.aliases?.length ? `, aliases: ${a.aliases.join(", ")}` : ""}): ${a.description}`)
			: ["- (none)"]),
		...(restrictedAgents.length ? [
			...
			...restrictedAgents.map((a) => `- ${a.name} (${a.source}, invocation: ${effectiveAgentInvocation(a)}${a.aliases?.length ? `, aliases: ${a.aliases.join(", ")}` : ""}): ${a.description}`),
		] : []),
		"",
		"Chains:",
