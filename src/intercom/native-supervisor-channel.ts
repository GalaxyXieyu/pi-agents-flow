...
function buildParentIntercomTool(pending: Map<string, PendingSupervisorRequest>, state: SubagentState, name = "intercom"): ToolDefinition<typeof IntercomParamsSchema, Record<string, unknown>> {
	return {
		...
		description: name === "intercom"
			? "Native pi-agents-flow supervisor channel. Use reply/pending/status to answer child subagent requests."
			: "Native pi-agents-flow supervisor channel. Use reply/pending/status to answer child subagent requests without overriding pi-intercom.",
		parameters: IntercomParamsSchema,
		async execute(_id, params) {
			...
			if (input.action === "send" || input.action === "ask") {
				throw new Error("Native pi-agents-flow intercom currently handles supervisor replies. Child agents initiate asks with contact_supervisor.");
			}
