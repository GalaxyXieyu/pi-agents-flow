...
export interface FleetTranscriptEventSpan {
	key: string;
	eventIndex: number;
	startLine: number;
	endLine: number;
	kind: FleetTranscriptEvent["kind"];
}

export interface RenderedFleetTranscript {
	lines: string[];
...
function hashEventText(value: string): string {
	let hash = 2166136261;
...
function fleetTranscriptEventBaseKey(event: FleetTranscriptEvent): string {
	if (event.kind === "tool" && event.toolCallId) return `tool:${event.toolCallId}`;
	if (event.kind === "tool") return `tool:${event.timestamp ?? "na"}:${event.name}:${hashEventText(event.argsPayload ?? event.args ?? "")}`;
	if (event.kind === "assistant" || event.kind === "user") return `${event.kind}:${event.timestamp ?? "na"}:${hashEventText(event.text)}`;
	return `notice:${event.timestamp ?? "na"}:${hashEventText(event.text)}`;
}

export function fleetTranscriptEventKey(event: FleetTranscriptEvent, index: number): string {
	const base = fleetTranscriptEventBaseKey(event);
	return event.timestamp === undefined && !(event.kind === "tool" && event.toolCallId) ? `${base}:${index}` : base;
}

function fleetTranscriptEventKeys(events: FleetTranscriptEvent[]): string[] {
	const bases = events.map(fleetTranscriptEventBaseKey);
	...
	return bases.map((base, index) => {
		...
		if (occurrence === 1) return fleetTranscriptEventKey(events[index]!, index);
		return `${base}:duplicate-${occurrence}`;
	});
}

function statusGlyph(event: Extract<FleetTranscriptEvent, { kind: "tool" }>, theme: Theme): string {
	return statusBadge(event.status === "error" ? "failed" : event.status === "running" ? "running" : "completed", theme);
}
...
export function renderFleetTranscriptView(
	transcript: FleetTranscript,
	width: number,
	theme: Theme,
	markdownTheme: MarkdownTheme,
	options: { expandedTools?: boolean; expandedToolKeys?: ReadonlySet<string>; selectedEventKey?: string } = {},
): RenderedFleetTranscript {
	if (width <= 0) return { lines: [], eventSpans: [] };
	const lines: string[] = [];
	const eventSpans: FleetTranscriptEventSpan[] = [];
	...
	const eventKeys = fleetTranscriptEventKeys(transcript.events);
	for (const [eventIndex, event] of transcript.events.entries()) {
		const eventKey = eventKeys[eventIndex]!;
		const startLine = lines.length;
		const finishEvent = (): void => {
			if (lines.length === startLine) return;
			if (options.selectedEventKey === eventKey) lines[startLine] = bounded(`${theme.fg("accent", ">")} ${lines[startLine] ?? ""}`, width);
			eventSpans.push({ key: eventKey, eventIndex, startLine, endLine: lines.length - 1, kind: event.kind });
		};
		if (event.kind === "tool") {
			const expanded = options.expandedTools || options.expandedToolKeys?.has(eventKey) === true;
			if (expanded && (event.output || event.argsPayload || event.error)) {
				lines.push(...renderExpandedTool(event, Math.max(1, width - 2), theme).map((line) => bounded(`  ${line}`, width)));
		...
		const marker = assistant ? theme.fg("accent", "[AI]") : theme.fg("warning", "[YOU]");
	...
	const lastSpan = eventSpans.at(-1);
	if (lastSpan && lastSpan.endLine >= lines.length) lastSpan.endLine = Math.max(lastSpan.startLine, lines.length - 1);
	return { lines, eventSpans };
}
...
export function renderFleetTranscript(
	...
	return renderFleetTranscriptView(transcript, width, theme, markdownTheme, options).lines;
}
