...
• Optional timeout: { timeoutMs } or { maxRuntimeMs } sets an explicit run-level max runtime for foreground and async/background runs; omitted means no runtime deadline unless the selected agent declares one
...
• {chain_dir} - Shared directory for chain files (e.g., <tmpdir>/pi-agents-flow-<scope>/chain-runs/abc123/)
...
• { action: "create", config: { name: "custom-agent", package: "code-analysis", systemPrompt, systemPromptMode, inheritProjectContext, inheritSkills, defaultContext, acceptance, acceptanceRole: "read-only" | "writer", visibility: "default" | "hidden", invocation: "both" | "model" | "user" | "disabled", ... } }
• acceptanceRole affects inferred acceptance only, never tool access. visibility controls model-facing catalogs; invocation controls trusted model/user launch origins. Explicit task mutation/no-edit intent wins; omission preserves name heuristics and policy defaults. Update an optional field with false or an empty string to clear it.
...
• context can be "fresh" or "fork"; omitted uses each agent defaultContext, otherwise fresh. timeoutMs/maxRuntimeMs apply to foreground and async/background runs; omitted means no runtime deadline unless the selected agent declares one.
...
• Agent acceptanceRole (read-only or writer) affects inferred acceptance only, never tools. visibility (default or hidden) controls model-facing catalogs; invocation (both, model, user, or disabled) controls trusted launch origins. Explicit task intent wins; omission keeps name heuristics and policy defaults. Update an optional field with false or an empty string to clear it.
...
function warn(options: ToolDescriptionOptions | undefined, message: string): void {
	(options?.warn ?? console.warn)(`[pi-agents-flow] ${message}`);
}
