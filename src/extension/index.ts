...
import { createActivityDockController, type ActivityDockController } from "../tui/activity-dock.ts";
...
import { createWorkflowController } from "../workflows/controller.ts";
import { createWorkflowDelegationAdapter } from "../workflows/delegation-adapter.ts";
import { createWorkflowRuntime } from "../workflows/runtime.ts";
import { noticePrefix, statusBadge } from "../tui/visual-language.ts";
import { registerWorkflowCommands } from "../workflows/commands.ts";
import { registerWorkflowAssetsTool, registerWorkflowTool } from "../workflows/tool.ts";
import { createWorkflowTodoAdapter, WORKFLOW_TODO_SNAPSHOT_ENTRY_TYPE } from "../workflows/todo-adapter.ts";
...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	let activityDock: ActivityDockController | undefined;
	...
	const workflowController = createWorkflowController({
		...
		onRunChanged: () => activityDock?.refresh(),
		defaultLanguage: config.workflowLanguage ?? "auto",
	});
	const workflowRuntime = createWorkflowRuntime({ pi, controller: workflowController });
	activityDock = createActivityDockController({
		getSnapshot: () => {
			const ctx = state.lastUiContext;
			let run: ReturnType<typeof workflowController.current>;
			try {
				run = ctx ? workflowController.current(ctx) : undefined;
			} catch {
				run = undefined;
			}
			return buildActivitySnapshot(state, run);
		},
		openSelection: (selection, perspective) => {
			const ctx = state.lastUiContext;
			if (!ctx?.hasUI) return;
			return openActivityBoard(ctx, state, () => {
				let run: ReturnType<typeof workflowController.current>;
				try { run = workflowController.current(ctx); } catch { run = undefined; }
				return buildActivitySnapshot(state, run);
	...
	registerSlashCommands(pi, state, {
		resolveLanguage: (ctx) => {
			try {
				const run = workflowController.current(ctx);
				if (run) return workflowRunLanguage(run);
			} catch {
				// Fleet remains available when no workflow is bound to this session.
			}
			return config.workflowLanguage === "zh" ? "zh" : "en";
		},
	});
	registerWorkflowCommands(pi, workflowController, {
		openActivity: async (ctx, runId) => {
			await openActivityBoard(ctx, state, () => {
				let run: ReturnType<typeof workflowController.current>;
				try { run = workflowController.current(ctx); } catch { run = undefined; }
				return buildActivitySnapshot(state, run);
