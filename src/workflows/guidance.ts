	...
	if (reviewerRelease && !evaluation.readyToComplete) {
		...
		actions.push({
			...
			promptHints: [
				...
				"If residual gaps/conflicts are deliberately acceptable, add a new reviewer of the final editor and instruct it (in agentSpec.instructions) to return top-level review={verdict:'pass'} and top-level extensions.release={release:true, gapsAccepted?, conflictsAccepted?, rationale} in its structured WorkflowResult — do not declare extensions.release on the planning dataContract (that field is decorative and never read for release gating). Do not replace the accepted reviewer.",
			],
		});
	}
