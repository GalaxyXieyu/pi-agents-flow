...
function withWorkflowContext(store: WorkflowStore, run: WorkflowRun, node: WorkflowRun["nodes"][string], outputSlots: Record<string, string> = {}): WorkflowRun["nodes"][string] {
	...
	const taskContext = [
		...
		...(node.kind === "reviewer" ? ["Review only the authorized final editor input. Do not broaden the research scope or call web/search tools by default.", "Always return a valid review result even when the document verdict is fail: the Supervisor should accept a sound review result, then repair or supersede the Editor draft based on its blockers. Reviewer node acceptance means the review was valid, not that the document passed.", "When the document verdict is pass, always return extensions.release={release:true, gapsAccepted?, conflictsAccepted?, citationShortfallAccepted?, lengthShortfallAccepted?, rationale}. Omit extensions.release when the verdict is fail. The release declaration both approves the final document and relaxes only the explicitly accepted residual gates. Be conservative: only release a gate when you have independently concluded the residual issue does not undermine the deliverable."] : []),
		...qualityContract,
	].join("\n\n");
