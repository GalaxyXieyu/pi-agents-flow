	...
	return [
		...
		mode === "deep-research"
			? [
				...
				"Design a detailed outline whose sections each define objective, questions, evidence requirements, target words, and explicit Section Writer ownership. Use writerNodeIds for ownership; the same Section Writer may own several sections. writerNodeId remains legacy compatibility only. Call set_outline with approval=user so Pi opens the inline approval/revision form; do not print a standalone confirmation request. If the user requests changes, revise the outline and call set_outline again. Use approval=supervisor only when the user explicitly requested immediate execution without outline review.",
