...
interface CreateWorkflowControllerOptions {
...
function bindingFrom(value: unknown): WorkflowBinding | undefined {
...
function latestBinding(ctx: ExtensionContext): WorkflowBinding | undefined {
...
function parseForegroundCompletion(value: unknown): SubagentForegroundCompleteEvent | undefined {
	if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
...
function guidanceDetails(run: WorkflowRun, evaluation: WorkflowEvaluation): { repairGuidance: WorkflowRepairGuidance; guidanceText: string } {
...
function codingWorkflowContract(run: WorkflowRun): CodingWorkflowContract | undefined {
	return run.codingContract;
}

function effectiveWorkflowNodeId(run: WorkflowRun, nodeId: string): string {
...
function statusText(run: WorkflowRun, evaluation: WorkflowEvaluation, includeResults = false): string {
	const lines = [
...
export function createWorkflowController(options: CreateWorkflowControllerOptions): WorkflowController {
	...
	const rootDir = options.rootDir ?? ((cwd: string) => path.join(cwd, ".pi-agents-flow", "workflows"));
