...
export function registerWorkflowTool(pi: ExtensionAPI, controller: WorkflowController, getSnapshot?: () => ActivitySnapshot | undefined): void {
	const tool: ToolDefinition<typeof WorkflowParams, WorkflowControllerDetails> = {
		...
		"Omit agentSpec.model on every node by default so children inherit the current session model with the workflow fallback chain. Set agentSpec.model only when a node explicitly needs a different tier (call workflow_assets first to see available fast/standard/deep models); never guess a model name that is not in the catalog.",
			...
			"Evidence quality defaults to policy.evidenceMode=auto and policy.qualityEnforcement=advisory: infer web/local/mixed behavior from accepted evidence and report shortfalls as warnings. Set strict only when the user explicitly requires a web, local, or mixed evidence contract; strict local accepts verified artifactPath/file evidence and never requires URL citations.",
			"Child completion is not acceptance. Call evaluate, then accept or reject each completed node with a concrete rationale. Reviewer results must use top-level review.verdict=pass|fail; only a valid pass with top-level extensions.release marks the reviewed document as approved for completion. Never call complete unless evaluate reports nextAction=complete and readyToComplete=true.",
			"An accepted node is immutable audit history and cannot be replaced. When an accepted Reviewer identifies document defects, add a new editor revision that depends on the accepted editor and review, then add a new reviewer that depends on the new editor; neither work unit uses replaces. To accept residual gaps/conflicts, instruct the final reviewer to return top-level review:{verdict:'pass'} plus explicit top-level extensions.release flags (such as gapsAccepted/conflictsAccepted); do not declare them on the plan's dataContract.extensions. The runtime format-steers malformed reviewer control fields to that same Agent before failing the attempt.",
