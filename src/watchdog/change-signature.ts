	...
	if (stat.size > maxHashFileBytes() || budget.bytes + stat.size > budget.maxBytes) {
		...
		try {
			...
			if (code !== "ERR_FS_FILE_TOO_LARGE") {
				console.warn("[pi-agents-flow] watchdog hashFile fell back to metadata for", normalized + ":", (error as Error)?.message);
			}
		}
	}
...
export function computeWatchdogRepoChangeSignature(cwd: string): WatchdogRepoChangeSignature | undefined {
	...
	try {
		...
		console.warn("[pi-agents-flow] watchdog repo change signature failed:", (error as Error)?.message);
		return undefined;
	}
}
