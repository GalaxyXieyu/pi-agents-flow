...
export interface PromptTemplateBridgeResult {
	...
	details?: {
		...
		results?: Array<{
			...
			acceptance?: { status: string; evidenceStatus?: string; explicit: boolean };
...
export function toSubagentDelegationExecutionParams(request: SubagentDelegationRequest): DelegatedSubagentExecutionParams {
	return {
		...
		skill: request.skill,
		...(request.extraTools ? { extraTools: request.extraTools } : {}),
		...(request.denyTools ? { denyTools: request.denyTools } : {}),
		output: false,
...
function resolveSubagentDelegationStatus(
	...
	if (child.execution?.status === "detached" || child.exitCode === -2) return "detached";
...
export function toSubagentDelegationResponse(
	...
	return {
		...
		...(child?.artifactPaths?.metadataPath ? { metadataPath: child.artifactPaths.metadataPath } : {}),
