...
import { BUILTIN_AGENT_NAMES, canInvokeAgent, discoverAgents, discoverAgentsAll, resolveAgentName, type AgentConfig, type ChainConfig } from "../agents/agents.ts";
import {
...
function resolveUserInvocableAgent(name: string, agents: AgentConfig[]): AgentConfig | undefined {
	const resolved = resolveAgentName(name, agents);
	return resolved.agent && canInvokeAgent(resolved.agent, "user") ? resolved.agent : undefined;
}

const makeAgentCompletions = (state: SubagentState, multiAgent: boolean) => (prefix: string) => {
	if (!state.baseCwd) return null;
	const agents = discoverAgents(state.baseCwd, "both").agents.filter((agent) => canInvokeAgent(agent, "user"));
	if (!multiAgent) {
...
class SubagentsStopSelector implements Component {
	...
	render(width: number): string[] {
		...
		lines.push("");
		lines.push(this.theme.fg("dim", "↑↓/jk select · Enter choose · Esc cancel"));
		return lines;
	}
}
...
const parseAgentArgs = (
	...
	for (const step of steps) {
		const resolved = resolveUserInvocableAgent(step.name, agents);
		if (!resolved) {
			ctx.ui.notify(`Unknown or non-user-invocable agent: ${step.name}`, "error");
			return null;
		}
		step.name = resolved.name;
	}
...
export function buildChainExpressionSteps(
	...
	for (const step of expression.steps) {
		const tasks = step.kind === "group" ? step.tasks : [step];
		for (const task of tasks) {
			const resolved = resolveUserInvocableAgent(task.name, agents);
			if (!resolved) {
				notify(`Unknown or non-user-invocable agent: ${task.name}`);
				return null;
			}
...
export function registerSlashCommands(
	pi: ExtensionAPI,
	state: SubagentState,
	options: { resolveLanguage?: (ctx: ExtensionContext) => "zh" | "en" } = {},
): void {
	let fleetOpen = false;
	const showFleet = async (ctx: ExtensionContext) => {
		state.lastUiContext = ctx;
		const language = options.resolveLanguage?.(ctx) ?? "en";
		if (!ctx.hasUI) {
			await runSlashSubagent(pi, ctx, { action: "status", view: "fleet" });
			return;
		}
		if (fleetOpen) {
			ctx.ui.notify(language === "zh" ? "子 Agent 平台已经打开。" : "Subagents Platform is already open.", "info");
			return;
		}
		fleetOpen = true;
		try {
			await openSubagentFleet(ctx, state, { language });
		} finally {
			fleetOpen = false;
		}
	};
	...
	pi.registerCommand("run", {
		...
		handler: async (args, ctx) => {
			...
			const agents = discoverAgents(state.baseCwd, "both").agents;
			const resolvedAgent = resolveUserInvocableAgent(agentName, agents);
			if (!resolvedAgent) { ctx.ui.notify(`Unknown or non-user-invocable agent: ${agentName}`, "error"); return; }

			let finalTask = task;
			if (inline.reads && Array.isArray(inline.reads) && inline.reads.length > 0) {
				finalTask = `[Read from: ${inline.reads.join(", ")}]\n\n${finalTask}`;
			}
			const params: SubagentParamsLike = { agent: resolvedAgent.name, task: finalTask, clarify: false, agentScope: "both" };
		...
		description: "Stop a current-session async subagent run",
				...
				if (id) {
					if (ctx.hasUI) {
						const confirmed = await ctx.ui.confirm(
							`Stop subagent run ${id}?`,
							"Stopping ends the run. Use interrupt when a resumable pause is required.",
						);
						if (!confirmed) return;
					}
			...
			if (targets.length === 0) {
				...
				const result = await ctx.ui.custom<StopSelectorResult>(
					...
					{ overlay: true, overlayOptions: { anchor: "center", width: "95%", minWidth: 40, maxWidth: 88, maxHeight: "80%" } },
				);
				if (!result?.confirmed || !result.target) return;
				const confirmed = await ctx.ui.confirm(
					`${result.target.actionLabel} ${result.target.id}?`,
					result.target.kind === "async"
						? "Stopping ends the run. Use interrupt when a resumable pause is required."
						: "Cancel this scheduled run before it starts.",
				);
				if (!confirmed) return;
	...
	pi.registerCommand("subagents-profiles", {
		...
		handler: async (_args, _ctx) => {
			...
			if (profiles.length === 0) {
				sendSlashText(pi, "Subagent profiles\n\nNo subagent profiles found in ~/.pi/agent/profiles/pi-agents-flow/");
				return;
			}
			sendSlashText(pi, `Subagent profiles\n\n${profiles.join("\n")}`);
		},
