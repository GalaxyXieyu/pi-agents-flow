...
export interface SubagentLaunchContractInput {
	...
	/**
	 * Per-launch tool grants layered on the base Agent's allowlist. Entries with `/`
	 * are MCP direct-tool selectors; others are builtin tool names. Never widens the
	 * capability ceiling.
	 */
	extraTools?: string[];
	/** Per-launch tool revocations, applied after the ceiling filter. */
	denyTools?: string[];
...
export interface SubagentLaunchContractTools {
	...
	/** Tools opened for this launch only, after ceiling filtering. */
	grantedTools?: string[];
	/** Tools closed for this launch only. */
	revokedTools?: string[];
	capabilityCeiling?: ResolvedSubagentCapabilityCeiling;
...
export async function resolveSubagentLaunchContract(input: SubagentLaunchContractInput): Promise<SubagentLaunchContractResult> {
	...
	if (resolvedSkills.missing.includes("pi-agents-flow")) {
		return { ok: false, code: "missing_skill", message: "The pi-agents-flow orchestration skill is not child-injectable.", diagnostics };
	}
	...
	try {
		toolPlan = resolvePiLaunchToolPlan({
			...
			...(input.extraTools?.length ? { extraTools: input.extraTools } : {}),
			...(input.denyTools?.length ? { denyTools: input.denyTools } : {}),
			cwd: effectiveCwd,
	...
	const contractBase: Omit<SubagentLaunchContract, "digest"> = {
		...
		tools: {
			...
			...(toolPlan.grantedTools.length > 0 ? { grantedTools: toolPlan.grantedTools } : {}),
			...(toolPlan.revokedTools.length > 0 ? { revokedTools: toolPlan.revokedTools } : {}),
