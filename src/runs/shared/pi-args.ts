...
import { STRUCTURED_OUTPUT_CAPTURE_ENV, STRUCTURED_OUTPUT_SCHEMA_ENV, STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV } from "./structured-output.ts";
import { TEMP_ROOT_DIR, type JsonSchemaObject, type LaunchResolvedChildExtensions, type ResolvedToolBudget } from "../../shared/types.ts";
...
export interface ResolvePiLaunchToolPlanInput {
	...
	mcpDirectTools?: string[];
	/**
	 * Per-launch tool grants layered on top of the base Agent's declaration.
	 *
	 * Entries containing `/` are treated as MCP direct-tool selectors
	 * (`server` or `server/tool`); other entries are builtin tool names.
	 * Extension paths are rejected by the caller: loading arbitrary extension
	 * files is a far larger grant than naming a tool.
	 *
	 * These are applied *before* capability-ceiling filtering, so the ceiling
	 * remains a hard cap that a launch can never widen.
	 */
	extraTools?: string[];
	/** Per-launch tool revocations, applied after every other resolution step. */
	denyTools?: string[];
	cwd?: string;
	requireReadTool?: boolean;
...
export interface PiLaunchToolPlan {
	...
	/** Tools granted for this launch only, after ceiling filtering. */
	grantedTools: string[];
	/** Tools revoked for this launch only. */
	revokedTools: string[];
...
export function projectLaunchResolvedChildExtensions(toolPlan: Pick<PiLaunchToolPlan, "runtimeExtensions" | "configuredExtensions" | "extensionArgs" | "disableAmbientExtensions">): LaunchResolvedChildExtensions {
...
export function resolvePiLaunchToolPlan(input: ResolvePiLaunchToolPlanInput): PiLaunchToolPlan {
	...
	// Per-launch grants split by shape: `/` means an MCP selector, everything else
	// is a builtin tool name. Both are merged in before the ceiling filter below,
	// so a grant can only ever open something the ceiling already permits.
	const extraToolEntries = [...new Set(input.extraTools ?? [])];
	...
	const mcpSelectors = input.mcpDirectTools === undefined && extraMcpSelectors.length === 0
		? undefined
		: [...new Set([...(input.mcpDirectTools ?? []), ...extraMcpSelectors])];
	const resolvedMcpSelections = capabilityCeiling?.denyExtensions ? [] : resolveMcpDirectToolSelections(mcpSelectors, input.cwd);
	...
	const grantedTools = effectiveToolAllowlist.filter((tool) => extraBuiltinTools.includes(tool)
		|| effectiveMcpSelections.some((selection) => selection.name === tool
			...
			&& !(input.mcpDirectTools ?? []).some((selector) => selection.selector === selector || selection.selector.startsWith(`${selector}/`))));
	const revokedTools = [...new Set([...ceilingFilteredBuiltinTools, ...ceilingFilteredMcpSelections.map((selection) => selection.name)])]
		.filter((tool) => denyToolSet.has(tool));
	...
	const capabilityAudit = capabilityCeiling ? {
		...
		requestedMcpToolCount: mcpSelectors?.length ?? 0,
		effectiveMcpTools,
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	if (input.structuredOutput) {
		...
		if (input.structuredOutput.submissionDir) env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV] = input.structuredOutput.submissionDir;
	}
