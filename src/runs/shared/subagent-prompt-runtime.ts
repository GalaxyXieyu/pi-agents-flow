...
const SUBAGENT_ORCHESTRATION_SKILL_NAME_PATTERN = /<name>\s*pi-agents-flow\s*<\/name>/;
...
export function stripSubagentOrchestrationSkill(prompt: string): string {
	return prompt
		.replace(/\n{0,2}<skill\s+name=["']pi-agents-flow["'][^>]*>[\s\S]*?<\/skill>\n{0,2}/g, "\n\n")
...
export function rewriteSubagentPrompt(
	...
	const submissionDir = process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV]?.trim();
	const structured = process.env[STRUCTURED_OUTPUT_CAPTURE_ENV]
		? `\n\n${STRUCTURED_OUTPUT_INSTRUCTIONS}${submissionDir ? `\nStructured output submission directory: ${submissionDir}` : ""}`
		: "";
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
	...
	const structuredSubmissionDir = process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV];
	if (structuredOutputPath && structuredSchemaPath) {
		...
		const registerTool = pi.registerTool as unknown as (tool: {
			...
			description: string;
			parameters: unknown;
			execute: (_id: string, params: StructuredOutputSubmission) => Promise<unknown>;
		}) => void;
		registerTool({
