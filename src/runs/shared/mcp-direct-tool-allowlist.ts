...
/**
 * Enumerate every MCP direct tool the current config and metadata cache can expose.
 *
 * Unlike `resolveMcpDirectToolSelections`, this takes no selector list: it asks for
 * every configured server so callers can present the full catalog of choices.
 */
export function listAvailableMcpDirectTools(cwd = process.cwd()): ResolvedMcpDirectToolSelection[] {
	try {
		const config = loadMcpConfig(cwd);
		const cache = loadMetadataCache();
		if (!cache) return [];
		return resolveDirectToolSelections(config, cache, getToolPrefix(config.settings?.toolPrefix), Object.keys(config.mcpServers));
	} catch {
		return [];
	}
}
