...
async function runSingleAttempt(
	...
	const result: SingleResult = withRunContext({
		...
		...(options.structuredOutput ? {
			structuredOutputSchemaPath: options.structuredOutput.schemaPath,
			structuredOutputPath: options.structuredOutput.outputPath,
		} : {}),
...
async function runSyncCompletion(
	...
	if (skillNames.some((skill) => skill.trim() === "pi-agents-flow") && missingSkills.includes("pi-agents-flow")) {
		return withRunContext({
			index: options.index ?? 0,
			agent: agentName,
			task,
			...
			messages: [],
			usage: emptyUsage(),
			error: "Skills not found: pi-agents-flow",
		}, options.context);
	}
