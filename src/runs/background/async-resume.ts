...
export function readAsyncRecoveryDescriptor(asyncDir: string | undefined): SteeringRecoveryDescriptor | undefined {
	...
	const allowedFields = new Set([
		"version", "launchContractDigest", "sourceRunId", "agentContract", "agent", "invocation", "sessionFile", "cwd", "model", "fallbackModels", "thinking", "tools", "extensions",
	...
	if (parsed.invocation !== undefined && parsed.invocation !== "both" && parsed.invocation !== "model" && parsed.invocation !== "user" && parsed.invocation !== "disabled") throw new Error(`Invalid async recovery descriptor '${descriptorPath}': invocation is invalid.`);
