...
export function readSteerCapabilityAt(filePath: string): SteerCapability | undefined {
	try {
		return parseSteerCapability(JSON.parse(fs.readFileSync(filePath, "utf-8")));
	} catch {
		return undefined;
	}
}
...
export function readSteerCapability(asyncDir: string, index: number): SteerCapability | undefined {
	return readSteerCapabilityAt(steerCapabilityPath(asyncDir, index));
}

export function consumeSteerAckAt(filePath: string): SteerAck | undefined {
	if (!fs.existsSync(filePath)) return undefined;
	let ack: SteerAck | undefined;
	try { ack = parseSteerAck(JSON.parse(fs.readFileSync(filePath, "utf-8"))); } catch { ack = undefined; }
	try { fs.rmSync(filePath, { force: true }); } catch { return undefined; }
	return ack;
}
