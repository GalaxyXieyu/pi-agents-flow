...
describe("renderSubagentResult fork indicator", () => {
	...
	it("routes running single-result detail to the live fleet view", () => {
		const widget = renderSubagentResult!({
		...
		const text = widget.render(120).join("\n");
		assert.match(text, /Live details: \/subagents-fleet/);
		assert.doesNotMatch(text, /impl-1/);
	});
	...
	it("keeps the running compact result glyph stable when progress changes", () => {
		...
		assert.equal(renderGlyph(1), "◐");
		assert.equal(renderGlyph(2), "◐");
	});
