...
describe("subagent profiles slash commands", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	it("lists saved profiles", async () => {
		await withIsolatedHome(async () => {
			const profilesDir = path.join(process.env.HOME!, ".pi", "agent", "profiles", "pi-agents-flow");
	...
	it("refreshes a provider model catalog", async () => {
		await withIsolatedHome(async () => {
			...
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".pi", "agent", "profiles", "pi-agents-flow", "providers", "openai-codex.models.json")), true);
		});
	});
	...
	it("generates provider profiles", async () => {
		await withIsolatedHome(async () => {
			...
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".pi", "agent", "profiles", "pi-agents-flow", "openai-codex.quota.json")), true);
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".pi", "agent", "profiles", "pi-agents-flow", "openai-codex.quality.json")), true);
		});
	});
...
describe("subagents admin slash command", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("treats agent and scope selector cancellation as a silent no-op", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-cancel-", async (root) => {
				const sent: unknown[] = [];
				const notifications: string[] = [];
	...
	it("reports duplicate names as ambiguous without UI", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-ambiguous-", async (root) => {
	...
	it("keeps same-source duplicate agents distinct in interactive pickers", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-same-source-", async (root) => {
	...
	it("updates a project agent model without dumping metadata first", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-model-", async (root) => {
	...
	it("updates a profile-managed user agent through settings instead of pinning frontmatter", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-profile-", async (root) => {
	...
	it("does not snapshot global disableThinking into an unrelated builtin override", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-global-thinking-", async (root) => {
	...
	it("offers and saves max thinking only when model metadata declares it", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-thinking-", async (root) => {
			const settingsPath = path.join(process.env.PI_CODING_AGENT_DIR!, "settings.json");
			fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
			fs.writeFileSync(settingsPath, JSON.stringify({
	...
	it("uses the inherited session model's thinking capabilities and warns on refresh failure", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-inherited-thinking-", async (root) => {
	...
	it("does not pin inherited settings when editing a project-owned field", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-cross-scope-", async (root) => {
				const agentPath = path.join(root, ".pi", "agents", "worker.md");
				fs.writeFileSync(agentPath, "---\nname: worker\ndescription: Test worker\n---\n\nDo work.\n", "utf-8");
				const settingsPath = path.join(process.env.HOME!, ".pi", "agent", "settings.json");
				fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
	...
	it("does not write definitions from PI_SUBAGENT_EXTRA_AGENT_DIRS", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-extra-", async (root) => {
				const extraDir = path.join(root, "extra-agents");
	...
	it("respects package-owned fields and uses settings only for unset package fields", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-package-", async (root) => {
	...
	it("edits a system prompt through Pi's native editor", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-prompt-", async (root) => {
				const agentPath = path.join(root, ".pi", "agents", "worker.md");
				fs.writeFileSync(agentPath, `---\nname: worker\ndescription: Test worker\n---\n\nOriginal prompt.\n`, "utf-8");
	...
	it("cancels native system-prompt editing without mutation", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-prompt-cancel-", async (root) => {
				const agentPath = path.join(root, ".pi", "agents", "worker.md");
				const original = `---\nname: worker\ndescription: Test worker\n---\n\nOriginal prompt.\n`;
				fs.writeFileSync(agentPath, original, "utf-8");
				const sent: unknown[] = [];
	...
	it("reports native-editor failures without rewriting the agent", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-editor-failure-", async (root) => {
				const agentPath = path.join(root, ".pi", "agents", "worker.md");
				const original = `---\nname: worker\ndescription: Test worker\n---\n\nOriginal prompt.\n`;
				fs.writeFileSync(agentPath, original, "utf-8");
				const sent: unknown[] = [];
	...
	it("shows metadata for a named subagent without requiring UI", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-agents-flow-admin-metadata-", async (root) => {
			fs.writeFileSync(path.join(root, ".pi", "agents", "auditor.md"), `---\nname: auditor\ndescription: Audit things\nthinking: false\n---\n\nAudit.\n`, "utf-8");
			const sent: unknown[] = [];
			const commands = registerAdmin(sent);
...
describe("subagents-doctor slash command", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("opens the native fleet from both the slash command and direct shortcut", async () => {
		...
		assert.match(rendered, /Subagents Platform/);
		assert.match(rendered, /Live · activity/);
	});
