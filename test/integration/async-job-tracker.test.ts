...
describe("async job tracker", { skip: !available ? "pi packages not available" : undefined }, () => {
	it("keeps lifecycle tracking independent of captured UI contexts", () => {
		...
		try {
			...
			assert.doesNotThrow(() => tracker.handleStarted({ id: "run-stale-context", asyncDir: path.join(asyncRoot, "run-stale-context"), agent: "worker" }));
			assert.equal(state.asyncJobs.has("run-stale-context"), true);
	...
	it("removes completed jobs after retention and requests a rerender", async () => {
		...
		try {
			...
			assert.equal(state.fleetJobs.get("run-1")?.status, "complete", "fleet history should outlive transient job cleanup");
			assert.equal(ui.widgets.length, 0, "the tracker must never render widgets itself");
		} finally {
			removeTempDir(asyncRoot);
		}
	});

	it("never renders widgets; the Activity Dock owns the bottom surface", () => {
		...
		try {
			...
			assert.equal(state.asyncJobs.size, 1, "lifecycle tracking must stay active");
			assert.equal(ui.widgets.length, 0, "the tracker must never register widgets");
			tracker.resetJobs(ui.ctx as never);
	...
	it("restores active async runs into the widget after reset", async () => {
		...
		try {
			...
			assert.equal(ui.widgets.length, 0, "restored jobs must not render legacy widgets");
	...
	it("polls changed status into tracked jobs without touching UI rendering", async () => {
		const asyncRoot = createTempDir("pi-async-job-tracker-");
		try {
			...
			assert.equal(ui.widgets.length, 0, "polling must not register widgets");
			fs.writeFileSync(path.join(runDir, "events.jsonl"), `${JSON.stringify({
				type: "subagent.control",
				channels: ["event"],
				event: {
			...
			writeStatus(3000, 1);
			await new Promise((resolve) => setTimeout(resolve, 40));
			assert.equal(state.asyncJobs.get("run-unchanged")?.toolCount, 1, "changed non-terminal status should be polled into the tracked job");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("schedules cleanup when polling observes a completed status without a completion event", async () => {
		...
		try {
			...
			assert.equal(state.asyncJobs.size, 0);
			assert.equal(ui.widgets.length, 0, "polling cleanup must not render widgets");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("repairs stale running jobs during polling", async () => {
		...
		try {
			...
			assert.equal(ui.widgets.length, 0, "stale repair must not render widgets");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("repairs started jobs whose runner dies before writing status", async () => {
		...
		try {
			...
			await waitForCondition(() => state.asyncJobs.size === 0, "startup-crash async job cleanup");
			...
			assert.equal(ui.widgets.length, 0, "startup-crash repair must not render widgets");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
	...
	it("cleans up jobs when status polling hits a terminal read error", async () => {
		...
		try {
			...
			assert.equal(state.asyncJobs.size, 0);
			assert.equal(ui.widgets.length, 0, "malformed status cleanup must not render widgets");
		} finally {
			removeTempDir(asyncRoot);
		}
	});
