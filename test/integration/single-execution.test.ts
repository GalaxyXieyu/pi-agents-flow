...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("keeps delegated invocation origin trusted", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		const executor = makeExecutor([makeAgent("manual", { invocation: "user" })]);
		const ctx = makeMinimalCtx(tempDir);

		const modelResult = await executor.executeDelegated(
			"model-delegated",
			{ agent: "manual", task: "Run" },
			new AbortController().signal,
			undefined,
			ctx,
		);
		assert.equal(modelResult.isError, true);
		assert.match(modelResult.content[0]?.text ?? "", /does not allow model invocation/);

		mockPi.onCall({ output: "manual finished" });
		const userResult = await executor.executeDelegated(
			"user-delegated",
			{ agent: "manual", task: "Run" },
			new AbortController().signal,
			undefined,
		...
		assert.equal(userResult.isError, undefined);
		assert.match(userResult.content[0]?.text ?? "", /manual finished/);
		assert.equal(mockPi.callCount(), 1);
	});

	it("rejects model-originated management mutation without native confirmation", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		const executor = makeExecutor([makeAgent("manual", { invocation: "user" })]);
		const result = await executor.execute(
			"policy-update",
			{ action: "update", agent: "manual", config: { invocation: "both" } },
			new AbortController().signal,
			undefined,
			makeMinimalCtx(tempDir),
		);
		...
		assert.match(result.content[0]?.text ?? "", /requires native user confirmation/);
		assert.equal(mockPi.callCount(), 0);
	});

	it("rejects model-originated persistent watchdog configuration without native confirmation", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		const executor = makeExecutor();
		const result = await executor.execute(
			"watchdog-update",
			{ action: "watchdog.configure", scope: "user", model: "inherit" },
			new AbortController().signal,
			undefined,
			makeMinimalCtx(tempDir),
		);
	...
	it("admits a zero run-level tool budget only for authorized delegation", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		const delegated = await executor.executeDelegated(
			"delegated-zero-budget",
			{ ...params, delegatedAllowZeroToolBudget: true },
			new AbortController().signal,
			undefined,
			ctx,
		...
		const env = JSON.parse(delegated.content[0]?.text ?? "{}") as Record<string, string>;
	...
	it("allows concurrent delegated foreground execution calls", async () => {
		...
		assert.equal(first.details?.runId, "first");
		assert.equal(second.details?.runId, "second");
		assert.equal(mockPi.callCount(), 2);
	});

	it("routes registered strict text delegation through the concurrent executor", async () => {
		...
		mockPi.onCall({ output: "registered second node", delay: 100 });
		...
		extensionEvents.on(SUBAGENT_DELEGATION_STARTED_EVENT, (payload) => {
			if ((payload as { version?: unknown }).version === SUBAGENT_DELEGATION_PROTOCOL_VERSION) {
		...
		try {
			...
			assert.deepEqual(started.map(({ requestId, ownerRunId, nodeId }) => ({ requestId, ownerRunId, nodeId })).sort((a, b) => a.nodeId.localeCompare(b.nodeId)), [
				{ requestId: "registered-a", ownerRunId: "owner", nodeId: "node-a" },
				{ requestId: "registered-b", ownerRunId: "owner", nodeId: "node-b" },
			]);
			...
			for (const response of terminalResponses) {
				assert.equal(response.ownerRunId, "owner");
			...
			const plainResponse = terminalResponses.find((response) => response.result?.kind === "text" && response.result.text === "registered second node");
