...
test("published extension APIs use supported package entrypoints", async () => {
	...
	assert.deepEqual(packageJson.exports, {
		".": "./index.ts",
		"./headless": "./src/extension/headless.ts",
	...
	const capability = await import("pi-agents-flow/capability-ceiling");
	assert.equal(capability.SUBAGENT_CAPABILITY_CEILING_VERSION, 1);
	assert.equal(capability.SUBAGENT_CAPABILITY_CEILING_REGISTRY_KEY, "pi-agents-flow.capability-ceiling.v1");
	const delegation = await import("pi-agents-flow/delegation");
	assert.equal(delegation.SUBAGENT_DELEGATION_PROTOCOL_VERSION, 2);
	assert.equal(delegation.SUBAGENT_DELEGATION_REQUEST_EVENT, "prompt-template:subagent:request");
	const preflight = await import("pi-agents-flow/preflight");
