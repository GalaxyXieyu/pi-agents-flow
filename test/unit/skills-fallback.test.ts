...
describe("skills filesystem fallback", () => {
	...
	it("does not expose pi-agents-flow as a child-injectable skill", () => {
		makeProjectSkill(tempDir, "pi-agents-flow", "Parent orchestration only.");
		makeProjectSkill(tempDir, "safe-bash", "Use safe bash.");

		const available = discoverAvailableSkills(tempDir).map((skill) => skill.name);
		assert.equal(available.includes("pi-agents-flow"), false);
		assert.equal(available.includes("safe-bash"), true);

		const { resolved, missing } = resolveSkills(["pi-agents-flow", "safe-bash"], tempDir);
		assert.deepEqual(missing, ["pi-agents-flow"]);
		assert.deepEqual(resolved.map((skill) => skill.name), ["safe-bash"]);

		const agentDir = path.join(tempDir, "agent");
		writeSkillFile(path.join(agentDir, "skills", "pi-agents-flow"), "Still parent-only.");
		const local = resolveSkills(["pi-agents-flow"], tempDir, ["./skills"], agentDir);
		assert.deepEqual(local.resolved, []);
		assert.deepEqual(local.missing, ["pi-agents-flow"]);
	});
