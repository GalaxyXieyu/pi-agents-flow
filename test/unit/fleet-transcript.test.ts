...
import { fleetTranscriptEventKey, readFleetTranscript, renderFleetTranscript, renderFleetTranscriptView } from "../../src/tui/fleet-transcript.ts";
...
describe("Fleet inspector structured transcript", () => {
	it("projects assistant Markdown, compact tools, errors, and later supervisor messages", () => {
		...
		try {
			...
			assert.ok(rendered.some((line) => line.includes("●") && line.includes("read") && line.includes("src/tui/fleet.ts")));
			...
			assert.ok(rendered.some((line) => line.includes("✕") && line.includes("write")));
	...
	it("returns stable event spans and expands only the selected tool", () => {
		const transcript = {
			path: "/trusted/transcript.jsonl",
			truncated: false,
			events: [
				{ kind: "tool" as const, toolCallId: "read-a", name: "read", argsPayload: JSON.stringify({ path: "a.ts" }), output: "alpha", status: "complete" as const },
				{ kind: "tool" as const, toolCallId: "read-b", name: "read", argsPayload: JSON.stringify({ path: "b.ts" }), output: "beta", status: "complete" as const },
			],
		};
		const selectedKey = fleetTranscriptEventKey(transcript.events[1]!, 1);
		const rendered = renderFleetTranscriptView(transcript, 52, theme as never, markdownTheme, {
			expandedToolKeys: new Set([selectedKey]),
			selectedEventKey: selectedKey,
		});
		assert.deepEqual(rendered.eventSpans.map((span) => span.key), ["tool:read-a", "tool:read-b"]);
		assert.ok(rendered.lines.some((line) => line.includes("alpha") && line.includes("x to expand")));
		assert.ok(rendered.lines.some((line) => line.includes("read b.ts")));
		assert.ok(rendered.lines.some((line) => line.includes("beta") && !line.includes("x to expand")));
		assert.equal(rendered.lines.filter((line) => line.includes("x to collapse")).length, 1);

		const duplicate = { kind: "tool" as const, name: "read", args: "same.ts", status: "running" as const };
		assert.notEqual(fleetTranscriptEventKey(duplicate, 0), fleetTranscriptEventKey(duplicate, 1));
		const singleView = renderFleetTranscriptView({ ...transcript, events: [duplicate] }, 52, theme as never, markdownTheme);
		const duplicateView = renderFleetTranscriptView({ ...transcript, events: [duplicate, duplicate] }, 52, theme as never, markdownTheme);
		assert.equal(new Set(duplicateView.eventSpans.map((span) => span.key)).size, 2);
		assert.equal(duplicateView.eventSpans[0]?.key, singleView.eventSpans[0]?.key, "the original event key must survive a duplicate append");
	});
