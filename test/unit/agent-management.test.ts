...
describe("agent management config parsing", () => {
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-management-"));
	...
	it("omits saved chains that reference non-model-invocable agents", () => {
		const agentsDir = path.join(tempDir, ".pi", "agents");
		...
		fs.writeFileSync(path.join(agentsDir, "manual.md"), "---\nname: manual\ndescription: Manual\ninvocation: user\n---\n\nManual.\n");
		fs.writeFileSync(path.join(agentsDir, "hidden-worker.md"), "---\nname: hidden-worker\ndescription: Hidden worker\nvisibility: hidden\ninvocation: model\n---\n\nHidden worker.\n");
		fs.writeFileSync(path.join(chainsDir, "visible-flow.chain.json"), JSON.stringify({
			name: "visible-flow",
			description: "Visible flow",
			chain: [{ agent: "visible", task: "Run" }],
		}));
		fs.writeFileSync(path.join(chainsDir, "manual-flow.chain.json"), JSON.stringify({
			name: "manual-flow",
			description: "Manual flow",
			chain: [{ agent: "manual", task: "Run" }],
		}));
		fs.writeFileSync(path.join(chainsDir, "hidden-worker-flow.chain.json"), JSON.stringify({
			name: "hidden-worker-flow",
			description: "Hidden worker flow",
			chain: [{ agent: "hidden-worker", task: "Run" }],
		}));

		const listed = readText(handleList(
			{ agentScope: "project" },
			{ cwd: tempDir, modelRegistry: { getAvailable: () => [] } },
		));
		assert.match(listed, /- visible-flow \(project\): Visible flow/);
		assert.match(listed, /- hidden-worker-flow \(project\): Hidden worker flow/);
		assert.doesNotMatch(listed, /manual-flow/);
	});
	...
	it("gets only the effective agent detail and respects explicit scope", () => {
		...
		fs.writeFileSync(path.join(packageDir, "package.json"), JSON.stringify({ "pi-agents-flow": { agents: ["agents"], chains: ["chains"] } }));
...
`, "utf-8");
	...
	it("creates, updates, reports, clears, and validates visibility and invocation policy", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const created = handleCreate(
			{ config: { name: "manual-agent", description: "Manual only", scope: "project", visibility: "hidden", invocation: "user" } },
			ctx,
		);
		assert.equal(created.isError, false);
		const filePath = path.join(tempDir, ".pi", "agents", "manual-agent.md");
		let content = fs.readFileSync(filePath, "utf-8");
		assert.match(content, /^visibility: hidden$/m);
		assert.match(content, /^invocation: user$/m);

		const got = readText(handleManagementAction("get", { agent: "manual-agent" }, ctx));
		assert.match(got, /Visibility: hidden/);
		assert.match(got, /Invocation: user/);
		assert.doesNotMatch(readText(handleManagementAction("list", {}, ctx)), /manual-agent/);

		const updated = handleUpdate({ agent: "manual-agent", config: { visibility: "default", invocation: "model" } }, ctx);
		assert.equal(updated.isError, false);
		content = fs.readFileSync(filePath, "utf-8");
		assert.match(content, /^visibility: default$/m);
		assert.match(content, /^invocation: model$/m);
		assert.match(readText(handleManagementAction("list", {}, ctx)), /manual-agent .*invocation: model/);

		assert.equal(handleUpdate({ agent: "manual-agent", config: { visibility: false, invocation: false } }, ctx).isError, false);
		content = fs.readFileSync(filePath, "utf-8");
		assert.doesNotMatch(content, /^visibility:/m);
		assert.doesNotMatch(content, /^invocation:/m);

		const invalidVisibility = handleUpdate({ agent: "manual-agent", config: { visibility: "private" } }, ctx);
		assert.equal(invalidVisibility.isError, true);
		assert.match(readText(invalidVisibility), /config\.visibility must be/);
		const invalidInvocation = handleUpdate({ agent: "manual-agent", config: { invocation: "automatic" } }, ctx);
		assert.equal(invalidInvocation.isError, true);
		assert.match(readText(invalidInvocation), /config\.invocation must be/);
	});
