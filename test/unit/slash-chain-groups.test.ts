...
before(() => {
	...
	fs.writeFileSync(
		path.join(agentsDir, "manual.md"),
		"---\nname: manual\ndescription: manual\naliases: manual-alias\nvisibility: hidden\ninvocation: user\n---\nBody\n",
		"utf-8",
	);
	fs.writeFileSync(
		path.join(agentsDir, "automatic.md"),
...
describe("buildChainExpressionSteps", () => {
	...
	it("resolves aliases and permits hidden user-only agents", () => {
		const notifications: string[] = [];
		const built = buildChainExpressionSteps(
			makeState(tempRoot) as never,
			'manual-alias "run manually" -> writer "finish"',
			makeCtx(notifications) as never,
		);
		assert.ok(built);
		assert.deepEqual(notifications, []);
		assert.equal(built?.chain[0] && "agent" in built.chain[0] ? built.chain[0].agent : undefined, "manual");
	});

	it("rejects model-only agents from user slash execution", () => {
		const notifications: string[] = [];
		const built = buildChainExpressionSteps(
			makeState(tempRoot) as never,
			'automatic "run" -> writer "finish"',
			makeCtx(notifications) as never,
		);
		assert.equal(built, null);
		assert.equal(notifications.length, 1);
		assert.match(notifications[0] ?? "", /non-user-invocable agent: automatic/);
	});
