...
describe("prompt-template delegation bridge", () => {
	it("uses trusted user origin for prompt templates and model origin for Workflow delegation requests", async () => {
		...
		const workflowOriginObserved = new Promise<void>((resolve) => { resolveWorkflowOrigin = resolve; });
		const bridge = registerPromptTemplateDelegationBridge({
			events,
			getContext: () => ({ cwd: "/repo" }),
			execute: async (_id, _params, _signal, _ctx, _onUpdate, origin) => {
				origins.push(origin);
				return { details: { results: [{ finalOutput: "legacy" }] } };
			},
			executeVersioned: async (_id, _params, _signal, _ctx, _onUpdate, origin) => {
				origins.push(origin);
				resolveWorkflowOrigin();
				return { details: { results: [{ finalOutput: "workflow" }] } };
			},
		});

		const legacyResponse = once(events, PROMPT_TEMPLATE_SUBAGENT_RESPONSE_EVENT);
		events.emit(PROMPT_TEMPLATE_SUBAGENT_REQUEST_EVENT, {
			requestId: "legacy-origin",
			agent: "worker",
		...
		await workflowOriginObserved;

		assert.deepEqual(origins, ["user", "model"]);
		bridge.dispose();
	});
