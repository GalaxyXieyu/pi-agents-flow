...
import { getCapabilities, setCapabilities, visibleWidth, type MarkdownTheme } from "@earendil-works/pi-tui";
import type { AgentConfig } from "../../src/agents/agents.ts";
import { collectFleetSnapshot, openSubagentFleet, SubagentFleetComponent, type FleetPromptRequest } from "../../src/tui/fleet.ts";
...
describe("native subagent fleet", () => {
	...
	it("renders structured tool activity and assistant Markdown when a child transcript is available", () => {
		...
		try {
			...
			try {
				...
				assert.ok(lines.some((line) => line.includes("Live") && line.includes("assistant response")));
				...
				try {
					...
					assert.notDeepEqual(lines, bottomLines, "K should scroll the conversation up by one line");
					component.handleInput("J");
					lines = component.render(100);
					assert.deepEqual(lines, bottomLines, "J should scroll the conversation back down by one line");
				} finally {
					Date.now = realDateNow;
				}
				for (let page = 0; page < 4; page++) component.handleInput("\x1b[5~");
				lines = component.render(100);
				assert.ok(lines.some((line) => line.includes("Live") && line.includes("assistant response")), "the live header should remain pinned while scrolling");
				assert.ok(lines.some((line) => line.includes("●") && line.includes("read") && line.includes("src/tui/fleet.ts")), "page up should reveal compact tool activity");
	...
	it("switches between live progress and a scrollable Agent definition card", () => {
		const state = stateForTest();
		state.foregroundControls.set("run-defined", {
			runId: "run-defined",
			mode: "single",
			...
			currentIndex: 0,
			description: "Investigate the language router",
		});
		const agent: AgentConfig = {
			name: "researcher",
			description: "Evidence-focused research employee.",
			systemPromptMode: "replace",
			inheritProjectContext: false,
			inheritSkills: false,
			systemPrompt: Array.from({ length: 20 }, (_, index) => `Research rule ${index + 1}`).join("\n"),
			source: "project",
			filePath: "/repo/.pi/agents/researcher.md",
			tools: ["read", "search"],
		};
		const component = new SubagentFleetComponent(
			{ terminal: { rows: 24, columns: 100 }, requestRender() {} } as never,
			theme as never,
			state,
			() => {},
			{ initialKey: "foreground-active:run-defined:0", refreshMs: 60_000, agents: [agent] },
		);
		const originalCapabilities = getCapabilities();
		try {
			setCapabilities({ images: null, trueColor: true, hyperlinks: true });
			let lines = component.render(100);
			assert.ok(lines.some((line) => line.includes("Live") && line.includes("follow")));
			assert.ok(lines.some((line) => line.includes(fleetIdentity("run-defined:0:researcher").name)));
			assert.ok(!lines.some((line) => line.includes("[Image:")), "unsupported terminals should omit the image block");

			setCapabilities({ images: "kitty", trueColor: true, hyperlinks: true });
			component.invalidate();
			lines = component.render(100);
			assert.ok(lines.some((line) => line.includes("\x1b_G")), "Kitty terminals should receive an inline PNG sequence");

			component.handleInput("v");
			lines = component.render(100);
			assert.ok(lines.some((line) => line.includes("Agent") && line.includes("definition")));
			assert.ok(lines.some((line) => line.includes("Evidence-focused research employee")));
			const firstPage = lines;
			component.handleInput("\x1b[6~");
			lines = component.render(100);
			assert.notDeepEqual(lines, firstPage, "PageDown should scroll the Agent definition");
			assert.ok(lines.some((line) => line.includes("System prompt") || line.includes("Research rule")));
		} finally {
	...
	it("toggles bounded tool output expansion with x and Ctrl+O", () => {
		...
		try {
			...
			try {
				...
				lines = component.render(100);
				assert.ok(lines.some((line) => line.includes("│>")), "detail focus should mark the selected event");
				...
				lines = component.render(100);
				assert.ok(!lines.some((line) => line.includes("│>") && line.includes("read")), "roster focus should clear the detail marker");
				fs.appendFileSync(
	...
	it("marks the inspector lifecycle so the Activity Dock can hide itself", async () => {
		const state = stateForTest();
		let widgetCalls = 0;
		...
		assert.equal(widgetCalls, 0, "the fleet inspector must not own any persistent status widget");
		assert.equal(observedOpen, true);
		assert.equal(state.fleetInspectorOpen, false);
	});

	it("uses native editor and confirmation outside the inspector overlay", async () => {
		const state = stateForTest();
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-fleet-native-prompts-"));
		try {
			const requests: Array<FleetPromptRequest | undefined> = [
				{ kind: "steer", itemKey: "async:run-1:0", runId: "run-1", asyncDir: root, index: 0, agent: "worker", source: "async" },
				{ kind: "stop", itemKey: "async:run-1:0", runId: "run-1", asyncDir: root, index: 0, agent: "worker", source: "async" },
				undefined,
			];
			const calls: unknown[] = [];
			...
			const ctx = {
				...
				ui: {
					setWidget() {},
					custom: async () => requests.shift(),
					editor: async (title: string) => {
						assert.equal(title, "Steer worker");
						return "Use the narrower approach";
					},
					confirm: async (title: string) => {
						assert.equal(title, "Stop worker?");
						return true;
					},
			...
			await openSubagentFleet(ctx as never, state, {
				actions: {
					async steer(input) { calls.push({ steer: input }); return { text: "Steering queued." }; },
					stop(input) { calls.push({ stop: input }); return { text: "Stop requested." }; },
				},
			});

			assert.deepEqual(calls, [
				{ steer: { runId: "run-1", asyncDir: root, index: 0, source: "async", message: "Use the narrower approach" } },
				{ stop: { runId: "run-1", asyncDir: root, index: 0 } },
			]);
			assert.deepEqual(notifications, [["Steering queued.", "success"], ["Stop requested.", "success"]]);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
	...
	it("focuses the selected child and renders raw foreground model details", () => {
		...
		state.foregroundControls.set("run-worker", {
			...
			recentTools: [{ tool: "search", args: "authentication", endMs: Date.now() - 2_000 }],
			recentOutput: ["Inspecting the authentication boundary"],
			toolCount: 2,
			tokens: 320,
		});
		...
		try {
			...
			assert.ok(selectedLine?.includes(fleetIdentity("run-worker:0:worker").name), `unexpected selected row: ${selectedLine}`);
			assert.ok(selectedLine?.includes("worker"), `selected row should preserve the real Agent role: ${selectedLine}`);
			assert.ok(lines.some((line) => line.includes("raw-model · thinking medium")));
			assert.ok(lines.some((line) => /Active\s+read src\/auth\.ts/.test(line)));
			assert.ok(lines.some((line) => line.includes("● search authentication")));
			assert.ok(lines.some((line) => line.includes("Inspecting the authentication boundary")));
		} finally {
			component.dispose();
		}
		...
		try {
			assert.ok(recentComponent.render(90).some((line) => line.includes("recent-raw-model · thinking xhigh")));
		} finally {
			recentComponent.dispose();
		}
	});

	it("requests the native editor for the selected async child", () => {
		...
		try {
			...
			const state = stateForTest();
			const requests: FleetPromptRequest[] = [];
			const component = new SubagentFleetComponent(
				{ terminal: { rows: 28, columns: 100 }, requestRender() {} } as never,
				theme as never,
				state,
				(request) => { if (request) requests.push(request); },
				{
					asyncDirRoot: root,
					...
					refreshMs: 60_000,
					actions: {
						async steer() { return { text: "Steering queued." }; },
						stop() {
							return { text: "unused" };
						},
					},
			...
			try {
				...
				assert.deepEqual(requests, [{ kind: "steer", itemKey: "async:async-steer:0", runId: "async-steer", asyncDir, index: 0, agent: "worker", source: "async" }]);
			} finally {
				component.dispose();
			}
		} finally {
	...
	it("requests native confirmation before stopping the selected async child", () => {
			...
			const component = new SubagentFleetComponent(
				...
				(request) => { if (request) requests.push(request); },
				{
					...
					actions: {
						...
						stop() { return { text: "Stop requested." }; },
					},
				},
			);
			try {
				component.handleInput("D");
				assert.deepEqual(requests, [{ kind: "stop", itemKey: "async:async-stop:0", runId: "async-stop", asyncDir, index: 0, agent: "worker", source: "async" }]);
			} finally {
				component.dispose();
			}
		} finally {
	...
	it("advertises steer only for a foreground child with a supported capability", () => {
		const state = stateForTest();
		const steeringDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-fleet-foreground-steer-"));
		const steerInboxDir = path.join(steeringDir, "targets", "0");
		const steerCapabilityPath = path.join(steeringDir, "capabilities", "0.json");
		const steerAckDir = path.join(steeringDir, "acks", "0");
		fs.mkdirSync(path.dirname(steerCapabilityPath), { recursive: true });
		fs.writeFileSync(steerCapabilityPath, JSON.stringify({ type: "steer-capability", protocolVersion: 1, index: 0, pid: process.pid, readyAt: Date.now(), supported: true }));
		state.foregroundControls.set("wf-child", {
			runId: "wf-child",
			mode: "single",
			startedAt: Date.now() - 1_000,
			updatedAt: Date.now(),
			currentAgent: "researcher",
			currentIndex: 0,
			description: "Investigate the router",
			steeringDir,
			activeChildren: new Map([[0, { index: 0, agent: "researcher", startedAt: Date.now() - 1_000, updatedAt: Date.now(), steerInboxDir, steerCapabilityPath, steerAckDir }]]),
		});
		const requests: FleetPromptRequest[] = [];
		const component = new SubagentFleetComponent(
			{ terminal: { rows: 28, columns: 100 }, requestRender() {} } as never,
			...
			(request) => { if (request) requests.push(request); },
			{
				initialKey: "foreground-active:wf-child:0",
				refreshMs: 60_000,
				actions: {
		...
		try {
			const footer = component.render(100).find((line) => line.includes("Tab focus"));
			assert.ok(footer, "the hint footer should render");
			assert.match(footer, /s stee/);
			assert.doesNotMatch(footer, /D stop/);
			component.handleInput("s");
			assert.deepEqual(requests, [{ kind: "steer", itemKey: "foreground-active:wf-child:0", runId: "wf-child", source: "foreground", index: 0, agent: "researcher" }]);
		} finally {
			component.dispose();
	...
	it("resumes follow at the tail and stays anchored when transcript events append", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-fleet-follow-tail-"));
		try {
			writeAsyncRun(root, {
				id: "async-follow",
				state: "complete",
			...
			try {
				let lines = component.render(100);
				assert.ok(lines.some((line) => line.includes("file-15.ts")));
				component.handleInput("\t");
				component.handleInput("K");
				lines = component.render(100);
				...
				fs.appendFileSync(
					path.join(root, "async-follow", "transcript-0.jsonl"),
					`${JSON.stringify({ recordType: "tool_start", toolCallId: "latest", toolName: "fetch_content", argsPreview: "LATEST-EVENT", ts: 99 })}\n`,
					"utf-8",
				);
				component.invalidate();
				lines = component.render(100);
				assert.ok(lines.some((line) => line.includes("LATEST-EVENT")), "follow should remain anchored after an append");
				assert.ok(lines.some((line) => line.includes("│>") && line.includes("LATEST-EVENT")), "follow should select the new tail event");
				assert.ok(lines.some((line) => line.includes("follow")));
			} finally {
				component.dispose();
			}
	...
	it("ignores Enter on non-tool events without pausing follow", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-fleet-assistant-enter-"));
		try {
			writeAsyncRun(root, {
				id: "async-assistant",
				state: "complete",
				transcript: [{ recordType: "message", role: "assistant", text: "Assistant tail", ts: 1 }],
			});
			const component = new SubagentFleetComponent(
				{ terminal: { rows: 24, columns: 100 }, requestRender() {} } as never,
				theme as never,
			...
			try {
				...
				component.handleInput("\r");
				const lines = component.render(100);
				assert.ok(lines.some((line) => line.includes("detail · follow")));
				assert.ok(!lines.some((line) => line.includes("Select a tool event")));
			} finally {
				component.dispose();
			}
		} finally {
	...
	it("uses Esc to return from detail but q to close immediately", () => {
		const state = stateForTest();
		state.foregroundControls.set("run-close", {
			runId: "run-close",
			mode: "single",
			startedAt: 1,
			updatedAt: 2,
	...
	it("keeps stacked narrow layouts within the overlay height budget", () => {
		const component = new SubagentFleetComponent(
			{ terminal: { rows: 10, columns: 60 }, requestRender() {} } as never,
			theme as never,
			stateForTest(),
			() => {},
			{ refreshMs: 60_000 },
		...
		try {
			assert.ok(component.render(60).length <= 10);
			(component as unknown as { tui: { terminal: { rows: number } } }).tui.terminal.rows = 6;
			assert.ok(component.render(60).length <= 6);
		} finally {
			component.dispose();
		}
	});
