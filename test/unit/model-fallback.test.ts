...
describe("model fallback helpers", () => {
	...
	it("treats bare provider transport failures as retryable so fallbacks are attempted", () => {
		// This exact message is what provider SDKs emit for a dropped socket. It used
		// to be classified as non-retryable, so a configured fallback model was never
		// tried and one flaky endpoint failed the node outright.
		assert.equal(isRetryableModelFailure("Connection error."), true);
		assert.equal(isRetryableModelFailure("APIConnectionError: Connection error."), true);
		assert.equal(isRetryableModelFailure("read ECONNRESET"), true);
		assert.equal(isRetryableModelFailure("connect ETIMEDOUT 1.2.3.4:443"), true);
		assert.equal(isRetryableModelFailure("getaddrinfo EAI_AGAIN api.example.com"), true);
		assert.equal(isRetryableModelFailure("write EPIPE"), true);
	});

	it("still refuses to switch models for a tool failure that merely mentions a connection", () => {
		// Retrying a different model cannot fix a failing tool call and would rerun
		// the entire task.
		assert.equal(isRetryableModelFailure("bash failed (exit 1): connection error"), false);
		assert.equal(isRetryableModelFailure("write failed with exit code 2"), false);
		assert.equal(isRetryableModelFailure('Validation failed for tool "structured_output"'), false);
		assert.equal(isRetryableModelFailure("Something ordinary went wrong"), false);
	});
