...
describe("registered subagent tool description", () => {
	...
	it("renders a custom project description with placeholders and mandatory safety guidance", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-project-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-agent-"));
	...
	it("appends full safety guidance when custom prose only includes the safety heading", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-heading-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-agent-"));
	...
	it("keeps mandatory safety guidance last when custom prose embeds it before an override", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-injection-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-agent-"));
		fs.mkdirSync(path.join(cwd, ".pi"), { recursive: true });
		fs.writeFileSync(
			path.join(cwd, ".pi", "subagent-tool-description.md"),
	...
	it("falls back to full mode when custom mode has no valid file", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-missing-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-agent-"));
		const warnings: string[] = [];
	...
	it("registers full, compact, custom, and fallback descriptions from extension config", () => {
		const defaultAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-default-"));
		assert.equal(readRegisteredDescription(defaultAgentDir), FULL_SUBAGENT_TOOL_DESCRIPTION);

		const compactAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-compact-"));
		...
		const customAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-custom-"));
		...
		const missingCustomAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-missing-"));
		...
		const invalidAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-tool-desc-invalid-"));
