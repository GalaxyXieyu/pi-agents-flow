...
describe("watchdog settings", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-watchdog-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-watchdog-project-"));
