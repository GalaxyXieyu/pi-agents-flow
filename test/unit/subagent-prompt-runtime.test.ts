...
import { STRUCTURED_OUTPUT_CAPTURE_ENV, STRUCTURED_OUTPUT_SCHEMA_ENV, STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV } from "../../src/runs/shared/structured-output.ts";
...
const envSnapshot = {
	...
	PI_SUBAGENT_STRUCTURED_OUTPUT_SUBMISSION_DIR: process.env.PI_SUBAGENT_STRUCTURED_OUTPUT_SUBMISSION_DIR,
...
afterEach(() => {
	...
	if (envSnapshot.PI_SUBAGENT_STRUCTURED_OUTPUT_SUBMISSION_DIR === undefined) delete process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV];
	else process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV] = envSnapshot.PI_SUBAGENT_STRUCTURED_OUTPUT_SUBMISSION_DIR;
...
describe("subagent prompt runtime", () => {
	...
	it("registers child watchdog lifecycle handlers only when enabled by env", () => {
		...
		// pi-agents-flow child, the runner sets it and an extra agent_end handler registers.
	...
	it("registered structured_output tool accepts valid schema output and writes the capture file", async () => {
		...
		try {
			...
			assert.deepEqual(parameters, {
				type: "object",
				properties: {
					value: { type: "object", required: ["ok"], properties: { ok: { type: "boolean" } } },
					path: { type: "string", minLength: 1, description: "Absolute or current-working-directory-relative path to a JSON file containing the complete result." },
					sha256: { type: "string", pattern: "^[A-Fa-f0-9]{64}$", description: "Optional SHA-256 digest of the referenced JSON file." },
				},
				oneOf: [
	...
	it("accepts a schema-valid JSON file reference without putting the result in tool arguments", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "subagent-structured-file-"));
		try {
			const schemaPath = path.join(dir, "schema.json");
			const outputPath = path.join(dir, "output.json");
			const submissionDir = path.join(dir, "submissions");
			const sourcePath = path.join(submissionDir, "large-result.json");
			const source = JSON.stringify({ ok: true, body: "x".repeat(100_000) });
			fs.writeFileSync(schemaPath, JSON.stringify({ type: "object", required: ["ok", "body"], properties: { ok: { type: "boolean" }, body: { type: "string" } } }), "utf-8");
			fs.mkdirSync(submissionDir);
			fs.writeFileSync(sourcePath, source, "utf-8");
			process.env[STRUCTURED_OUTPUT_SCHEMA_ENV] = schemaPath;
			process.env[STRUCTURED_OUTPUT_CAPTURE_ENV] = outputPath;
			process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV] = submissionDir;
			let execute: ((_id: string, params: { value?: unknown; path?: string; sha256?: string }) => Promise<{ terminate?: boolean }>) | undefined;
			registerSubagentPromptRuntime({
				registerTool(tool: { name: string; execute: typeof execute }) {
					if (tool.name === "structured_output") execute = tool.execute as NonNullable<typeof execute>;
				},
				on() {},
			} as never);
			assert.ok(execute);
			const result = await execute("tool-2", { path: sourcePath });
			assert.equal(result.terminate, true);
			assert.equal(fs.readFileSync(outputPath, "utf-8"), source);
		} finally {
			fs.rmSync(dir, { recursive: true, force: true });
		}
	});

	it("rejects unsafe or invalid file references without replacing a valid capture", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "subagent-structured-file-errors-"));
		try {
			const schemaPath = path.join(dir, "schema.json");
			const outputPath = path.join(dir, "output.json");
			const submissionDir = path.join(dir, "submissions");
			const sourcePath = path.join(submissionDir, "result.json");
			fs.writeFileSync(schemaPath, JSON.stringify({ type: "object", required: ["ok"], properties: { ok: { type: "boolean" } } }), "utf-8");
			fs.mkdirSync(submissionDir);
			fs.writeFileSync(sourcePath, JSON.stringify({ ok: true }), "utf-8");
			process.env[STRUCTURED_OUTPUT_SCHEMA_ENV] = schemaPath;
			process.env[STRUCTURED_OUTPUT_CAPTURE_ENV] = outputPath;
			process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV] = submissionDir;
			let execute: ((_id: string, params: { value?: unknown; path?: string; sha256?: string }) => Promise<unknown>) | undefined;
			registerSubagentPromptRuntime({
				registerTool(tool: { name: string; execute: typeof execute }) {
					if (tool.name === "structured_output") execute = tool.execute as NonNullable<typeof execute>;
				},
			...
			assert.ok(execute);
			await execute("tool-3", { path: sourcePath });
			await assert.rejects(() => execute!("tool-4", { path: path.join(dir, "missing.json") }), /Failed to resolve structured output file/);
			await assert.rejects(() => execute!("tool-5", { path: sourcePath, sha256: "0".repeat(64) }), /SHA-256 mismatch/);
			await assert.rejects(() => execute!("tool-6", { value: { ok: true }, path: sourcePath }), /exactly one/);
			fs.writeFileSync(sourcePath, JSON.stringify({ ok: "invalid" }), "utf-8");
			await assert.rejects(() => execute!("tool-7", { path: sourcePath }), /Structured output validation failed/);
			assert.deepEqual(JSON.parse(fs.readFileSync(outputPath, "utf-8")), { ok: true });
		} finally {
			fs.rmSync(dir, { recursive: true, force: true });
		}
	...
	it("does not allow a file reference outside the child working directory", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "subagent-structured-file-boundary-"));
		try {
			const schemaPath = path.join(dir, "schema.json");
			const outputPath = path.join(dir, "output.json");
			const submissionDir = path.join(dir, "submissions");
			const sourcePath = path.join(os.tmpdir(), `structured-outside-${process.pid}.json`);
			...
			process.env[STRUCTURED_OUTPUT_CAPTURE_ENV] = outputPath;
			process.env[STRUCTURED_OUTPUT_SUBMISSION_DIR_ENV] = submissionDir;
			let execute: ((_id: string, params: { path?: string }) => Promise<unknown>) | undefined;
			registerSubagentPromptRuntime({
				registerTool(tool: { name: string; execute: typeof execute }) {
					if (tool.name === "structured_output") execute = tool.execute as NonNullable<typeof execute>;
				},
			...
			assert.ok(execute);
			await assert.rejects(() => execute!("tool-8", { path: sourcePath }), /must be inside/);
		} finally {
			try { fs.unlinkSync(path.join(os.tmpdir(), `structured-outside-${process.pid}.json`)); } catch {}
			fs.rmSync(dir, { recursive: true, force: true });
		}
	});
	...
	it("strips the subagent orchestration skill even when inherited skills remain", () => {
		...
		assert.ok(rewritten.includes("<name>safe-bash</name>"));
		assert.ok(!rewritten.includes("<name>pi-agents-flow</name>"));
		assert.ok(!rewritten.includes("delegate to subagents"));
	});

	it("strips explicit pi-agents-flow skill injection from child prompts", () => {
		const prompt = "Before\n\n<skill name=\"pi-agents-flow\">\nDo not keep this.\n</skill>\n\n<skill name=\"safe-bash\">\nKeep this.\n</skill>\nAfter";
