...
describe("PI_CODING_AGENT_DIR runtime paths", () => {
	...
	it("resolves the agent dir dynamically and loads extension config from it", () => {
		...
		writeFile(configPath, JSON.stringify({ asyncByDefault: true, maxSubagentDepth: 3, artifactDir: "session", workflowLanguage: "zh", workflowMaxNodeAttempts: 5 }));

		const config = loadConfig();
		assert.equal(config.asyncByDefault, true);
		assert.equal(config.maxSubagentDepth, 3);
		assert.equal(config.artifactDir, "session");
		assert.equal(config.workflowLanguage, "zh");
		assert.equal(config.workflowMaxNodeAttempts, 5);
	});
...
`);
	...
	it("rejects invalid workflowMaxNodeAttempts config values", () => {
		const configPath = path.join(agentDir, "extensions", "subagent", "config.json");
		for (const value of [0, 21, 1.5, "3"]) {
			writeFile(configPath, JSON.stringify({ workflowMaxNodeAttempts: value }));
			assert.deepEqual(loadConfig(), {});
		}
	});

	it("rejects invalid workflowLanguage config values", () => {
		const configPath = path.join(agentDir, "extensions", "subagent", "config.json");
		writeFile(configPath, JSON.stringify({ workflowLanguage: "fr" }));

		assert.throws(() => updateConfig((config) => config), /config\.workflowLanguage must be "auto", "zh", or "en"/);
	});
	...
	it("uses the configured agent dir for subagent bridge instruction files", () => {
		...
		assert.equal(diagnostic.extensionDir, "native:pi-agents-flow-supervisor-channel");
		...
		assert.equal(bridge.extensionDir, "native:pi-agents-flow-supervisor-channel");
