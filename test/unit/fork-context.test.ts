...
describe("createForkContextResolver", () => {
	...
	it("opens a throwaway manager from the persisted parent session file", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-open-"));
		try {
	...
	it("creates forked sessions through the default package opener", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-default-"));
		try {
	...
	it("fails clearly for an unflushed user-only parent", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-user-only-"));
		try {
			const sessionDir = path.join(tempDir, "sessions");
			const parent = SessionManager.create(tempDir, sessionDir);
	...
	it("creates isolated branched sessions per index (parallel and chain compatible)", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-index-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			let count = 0;
	...
	it("memoizes per index to keep behavior deterministic", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-memo-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			let calls = 0;
	...
	it("persists a forked session when Pi defers writing the branched file", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-deferred-child-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("removes signed Anthropic thinking blocks before forwarding a forked session", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("keeps requested thinking for forked children when the force-off predicate declines", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-keep-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("still forces thinking off when the predicate confirms an Anthropic child", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-confirm-off-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("allows unsigned thinking blocks in forked sessions", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-unsigned-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("allows non-Anthropic signed thinking blocks in forked sessions", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-openai-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("removes redacted thinking blocks before fallback-persisting a forked session", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-redacted-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("fails clearly when branch extraction returns a missing child file without fallback state", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-missing-child-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("does not silently fallback to fresh when branch extraction fails", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-fork-no-path-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			const resolver = createForkContextResolver({
