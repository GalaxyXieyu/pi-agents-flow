...
function withTempHome<T>(fn: (home: string) => T): T {
	const home = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-package-home-"));
	tempDirs.push(home);
...
describe("agent skillPath frontmatter", () => {
	it("parses and serializes comma-separated paths", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-skill-path-agent-"));
...
describe("agent aliases", () => {
	it("parses and serializes agent aliases", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-alias-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".pi", "agents", "worker.md"), `---
name: worker
description: Worker
...
body`);
	...
	it("reports management alias collisions as ambiguous", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-alias-collision-"));
		tempDirs.push(project);
...
describe("agent simple-scalar list frontmatter", () => {
	it("discovers newline block lists for all list fields and routes MCP tools", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-block-list-frontmatter-"));
		tempDirs.push(dir);
...
`);
	...
	it("preserves MCP-only tools as an explicit empty builtin allowlist", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-mcp-only-frontmatter-"));
		tempDirs.push(dir);
...
`);
	...
	it("preserves comma-separated syntax across all list fields", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-comma-list-frontmatter-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".pi", "agents", "worker.md"), `---
name: worker
description: Worker
...
describe("agent permission frontmatter", () => {
	it("preserves nested permission YAML blocks through discovery and serialization", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-permission-frontmatter-"));
		tempDirs.push(dir);
...
describe("agent frontmatter defaultContext", () => {
	...
	it("parses defaultContext from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-default-context-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
`, "utf-8");
	...
	it("loads packaged planner, worker, oracle, and advisor with fork defaultContext", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-default-context-"));
		tempDirs.push(dir);
...
describe("agent frontmatter launch defaults", () => {
	it("serializes and discovers single-agent launch defaults", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-launch-defaults-"));
		tempDirs.push(dir);
	...
	it("parses scalar acceptance defaults and rejects invalid policies", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-acceptance-defaults-"));
		tempDirs.push(dir);
		const filePath = path.join(dir, ".pi", "agents", "worker.md");
		writeAgent(filePath, `---
name: worker
...
`);
	...
	it("parses, serializes, and validates acceptance roles", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-acceptance-role-"));
		tempDirs.push(dir);
...
`);
	...
	it("rejects invalid launch defaults instead of silently ignoring them", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-invalid-launch-defaults-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".pi", "agents", "worker.md"), `---
name: worker
description: Worker
...
describe("chain discovery", () => {
	it("prefers same-scope .chain.json over .chain.md for the same runtime name", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-chain-format-precedence-"));
		tempDirs.push(dir);
...
describe("package-provided agents and chains", () => {
	it("discovers package agents and chains from installed package manifests", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-package-discovery-"));
		tempDirs.push(dir);
...
`);
	...
	it("loads packages referenced from Pi settings", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-settings-package-"));
		tempDirs.push(dir);
...
`);
	...
	it("discovers project package agents when cwd is nested below the project root", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-nested-package-discovery-"));
		tempDirs.push(dir);
...
`);
	...
	it("keeps nearest project root discovery by default when a nested .pi exists", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-nearest-root-default-"));
		tempDirs.push(dir);
		const nested = path.join(dir, "packages", "app", "src");
...
`);
	...
	it("can resolve project packages and overrides from the git root", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-git-root-resolution-"));
		tempDirs.push(dir);
		const nested = path.join(dir, "packages", "app", "src");
		const nestedConfigDir = path.join(dir, "packages", "app", ".pi");
		const packageRoot = path.join(dir, ".pi", "npm", "node_modules", "outer-workflow");
...
`);
	...
	it("does not register legacy skill files from broad package agent roots", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-broad-package-skills-"));
		tempDirs.push(dir);
...
`);
	...
	it("prunes repo internals and nested project roots from broad package discovery", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-broad-package-prune-"));
		tempDirs.push(dir);
...
`);
	...
	it("keeps package definitions below user and project overrides", () => withTempHome((home) => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-package-precedence-"));
		tempDirs.push(dir);
...
`);
	...
	it("does not allow management updates to package agents", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-package-readonly-"));
		tempDirs.push(dir);
...
describe("agent frontmatter completionGuard", () => {
	...
	it("parses completionGuard from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-completion-guard-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter maxSubagentDepth", () => {
	...
	it("parses maxSubagentDepth from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter thinking", () => {
	it("coerces frontmatter false strings to disabled thinking", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-thinking-false-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`, "utf-8");
	...
	it("preserves supported frontmatter thinking strings", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-thinking-levels-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter fallbackModels", () => {
	...
	it("parses fallbackModels from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-fallback-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter systemPromptMode", () => {
	...
	it("parses systemPromptMode from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-prompt-mode-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter prompt inheritance flags", () => {
	...
	it("parses inheritProjectContext and inheritSkills from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-prompt-inheritance-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter subagentOnlyExtensions", () => {
	...
	it("parses subagentOnlyExtensions from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-child-ext-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter prompt assembly defaults", () => {
	it("defaults ordinary agents to replace mode with no inherited context or skills", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-default-prompt-settings-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
`, "utf-8");
	...
	it("builtin agents inherit project context by default", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-default-prompt-settings-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-default-home-"));
		tempDirs.push(dir);
	...
	it("bundled agents all have explicit tool allowlists", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-tools-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-tools-home-"));
		tempDirs.push(dir);
		tempDirs.push(homeDir);
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("keeps the bundled planner read-only for repository tools", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-planner-tools-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-planner-home-"));
		tempDirs.push(dir);
		tempDirs.push(homeDir);
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("worker and delegate include the child-facing supervisor tool", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-supervisor-tool-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-builtin-supervisor-tool-home-"));
		tempDirs.push(dir);
		tempDirs.push(homeDir);
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("defaults delegate to append mode with inherited project context", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-agent-delegate-default-prompt-settings-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("packaged agent and chain discovery", () => {
	it("recursively discovers nested project agents while keeping chain files separate", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-recursive-agent-discovery-"));
		tempDirs.push(dir);
...
`, "utf-8");
	...
	it("registers packaged agents by runtime name and serializes local name plus package", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-packaged-agent-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "scout.md"), `---
...
`, "utf-8");
	...
	it("recursively discovers packaged chains by runtime name and preserves package on serialize", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-packaged-chain-"));
		tempDirs.push(dir);
...
`;
	...
	it("keeps packaged and un-packaged runtime names distinct while preserving un-packaged precedence", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-packaged-collisions-"));
		tempDirs.push(dir);
...
`, "project", "/tmp/review.chain.md");
	...
	it("normalizes package frontmatter consistently for agents and chains", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-package-normalize-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		const chainsDir = path.join(dir, ".pi", "chains");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`, "utf-8");
	...
	it("skips invalid package frontmatter that cannot be normalized", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-invalid-package-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		const chainsDir = path.join(dir, ".pi", "chains");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("project agent directory discovery", () => {
	it("discovers project agents from both .agents and .pi/agents", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-agent-dirs-"));
		tempDirs.push(dir);
...
`, "utf-8");
	...
	it("does not register legacy project skill files as agents", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-skills-not-agents-"));
		tempDirs.push(dir);
...
`);
	...
	it("does not register user SKILL.md files as agents", () => withTempHome((home) => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-user-skills-not-agents-"));
		tempDirs.push(dir);
...
`);
	...
	it("prefers .pi/agents over .agents on project agent name collisions", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-agent-collision-"));
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".agents"), { recursive: true });
		fs.mkdirSync(path.join(dir, ".pi", "agents"), { recursive: true });
...
`, "utf-8");
	...
	it("uses the project root for the canonical project agent dir even when only .agents exists", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-agent-root-"));
		tempDirs.push(dir);
	...
	it("discovers project chains from .pi/chains", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-chain-dirs-"));
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".pi", "agents"), { recursive: true });
...
`, "utf-8");
	...
	it("prefers project .pi/chains over user chains on name collisions", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-project-chain-collision-"));
		const home = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-user-chain-home-"));
