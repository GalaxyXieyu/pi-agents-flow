...
describe("subagent extension child mode", () => {
	...
	it("registers only subagent_wait and honors waitTool disabled config", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-wait-tool-config-"));
		try {
	...
	it("never renders the legacy async widget; the Activity Dock owns the bottom surface", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-async-widget-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
			...
			const script = String.raw`
				...
				if (asyncWidgets.length !== 0) throw new Error("legacy async widget must never render: " + JSON.stringify(asyncWidgets));
				const dockWidgets = widgets.filter((entry) => entry.key === "pi-agents-flow-activity-dock" && entry.value !== undefined);
				if (dockWidgets.length === 0) throw new Error("activity dock did not register");
	...
	it("disposes pending completion notifications on session shutdown", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-agents-flow-notify-shutdown-"));
		const configDir = path.join(agentDir, "extensions", "subagent");
		fs.mkdirSync(configDir, { recursive: true });
