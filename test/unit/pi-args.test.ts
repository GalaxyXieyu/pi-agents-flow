...
describe("buildPiArgs session wiring", () => {
	...
	it("passes foreground steering transport paths to the child runtime", () => {
		const { env } = buildPiArgs({
			...
			steerAckDir: "/tmp/steer/acks",
		});
		assert.equal(env[SUBAGENT_STEER_INBOX_ENV], "/tmp/steer/inbox");
		assert.equal(env[SUBAGENT_STEER_CAPABILITY_ENV], "/tmp/steer/capability.json");
		assert.equal(env[SUBAGENT_STEER_ACK_DIR_ENV], "/tmp/steer/acks");
	});
