...
describe("async recovery descriptor", () => {
	...
	it("preserves and validates invocation policy", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-async-recovery-invocation-"));
		try {
			const descriptor = {
				version: 1,
				sourceRunId: "run-invocation",
				agent: "worker",
				invocation: "user",
				cwd: root,
				systemPromptMode: "replace",
			...
			fs.writeFileSync(path.join(root, "recovery-descriptor.json"), JSON.stringify(descriptor), "utf-8");
			assert.equal(readAsyncRecoveryDescriptor(root)?.invocation, "user");

			fs.writeFileSync(path.join(root, "recovery-descriptor.json"), JSON.stringify({ ...descriptor, invocation: "automatic" }), "utf-8");
			assert.throws(() => readAsyncRecoveryDescriptor(root), /invocation is invalid/);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
