...
function withTempHome<T>(fn: (home: string) => T): T {
	const home = mkdtemp("pi-agents-flow-mem-home-");
...
function mkProject(): string {
	const dir = mkdtemp("pi-agents-flow-mem-project-");
...
describe("resolveMemoryDir", () => {
	it("resolves a simple and nested path under the root", () => {
		const root = mkdtemp("pi-agents-flow-mem-root-");
	...
	it("rejects a symlinked ancestor before prompting a first write", () => {
		const root = mkdtemp("pi-agents-flow-mem-root-");
		const outside = mkdtemp("pi-agents-flow-mem-outside-");
...
describe("readMemoryFile", () => {
	it("returns null when no memory file exists", () => {
		const dir = mkdtemp("pi-agents-flow-mem-dir-");
	...
	it("rejects a symlinked memory file that escapes the memory dir", () => {
		const dir = mkdtemp("pi-agents-flow-mem-dir-");
		const outsideFile = path.join(mkdtemp("pi-agents-flow-mem-outside-"), "secret.md");
...
describe("buildAgentMemoryInjection", () => {
	...
	it("returns empty for project scope when no project root is found", () => {
		const nowhere = mkdtemp("pi-agents-flow-mem-noroot-");
