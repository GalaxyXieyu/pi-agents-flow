...
`);
		...
		try {
			...
			assert.equal(result.contract.roots.outputPath, path.join(cwd, ".pi-agents-flow", "artifacts", "outputs", "run-123", "report.md"));
			...
			assert.equal(fs.existsSync(path.join(cwd, ".pi-agents-flow")), false);
		} finally {
			handle.dispose();
		}
	});
