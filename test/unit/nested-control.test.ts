...
describe("nested control routing", () => {
	...
	it("routes resume for live nested runs through the control inbox", async () => {
		...
		try {
			...
			const executor = createExecutor(stateWithNestedRoute(route), [{ name: "worker", description: "Worker", prompt: "Do work" }], true, events);
			setTimeout(() => {
				const request = readNestedControlRequests(route)[0];
	...
	it("rejects model resume for a live user-only nested agent", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-nested-live-resume-policy-"));
		try {
			const route = createNestedRun("nested-live-resume-policy", "running");
			const result = await createExecutor(stateWithNestedRoute(route), [{ name: "worker", description: "Worker", prompt: "Do work", invocation: "user" }])
				.execute("resume", { action: "resume", id: "nested-live-resume-policy", message: "continue" }, new AbortController().signal, undefined, ctx(root));

			assert.equal(result.isError, true);
			assert.match(text(result), /does not allow model invocation/);
