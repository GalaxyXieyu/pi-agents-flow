...
11. If that accepted Reviewer finds document defects, append a new Editor revision depending on the accepted Editor and review, followed by a new Reviewer of that revision. Neither node uses `replaces`, because accepted nodes are immutable audit history. If residual gaps/conflicts are acceptable, the new Reviewer must set the relevant `extensions.release` flags explicitly.
