...
- An accepted node is immutable audit history, never a `replaces` target. An accepted Reviewer that finds document defects requires a new editor revision depending on the accepted editor and review, then a new reviewer depending on that editor; neither uses `replaces`. A new reviewer may explicitly accept residual gaps/conflicts by returning `extensions.release` (e.g. `{release:true, gapsAccepted, conflictsAccepted, rationale}`) in its result when it passes — do not declare it on the plan's `dataContract.extensions`; but it does not replace the accepted reviewer.
