...
- An accepted node is immutable audit history, never a `replaces` target. An accepted Reviewer that finds document defects requires a new editor revision depending on the accepted editor and review, then a new reviewer depending on that editor; neither uses `replaces`. A Reviewer must return top-level `review:{verdict:"pass"|"fail"}` in its structured WorkflowResult. A pass must also return top-level `extensions.release` (e.g. `{release:true, gapsAccepted, conflictsAccepted, rationale}`); a fail must omit it. The runtime format-steers a missing or malformed reviewer control envelope back to the same Agent before failing the attempt. Do not declare release data on the plan's `dataContract.extensions`; it is decorative and never read for release gating.
- Keep one writer per cwd.
- `dataContract.profile` is fixed by the node `kind`, not freely chosen: `research`/`verification` -> `research`, `section-writer`/`writer`/`editor` -> `writer`, `reviewer` -> `reviewer`, `outline`/`custom` -> `generic`. Custom roles (planner, checker, orchestrator, ...) are `kind: "custom"` + `profile: "generic"` even when they produce a document; `generic` means the runtime applies no built-in role output contract. The full mapping is in `references/agent-factory.md`.
...
- Every node already receives the runtime-default tools documented in `references/agent-factory.md`, including workspace read/write, web retrieval, and `contact_supervisor`. Do not repeat them in `extraTools` or generate `denyTools` for them during ordinary planning. Use `denyTools` only for a named read-only, offline, review-scope, or security restriction. Extensions remain inherited from the selected base Agent; node `extraTools` adds only role-specific builtin or MCP capabilities, and the capability ceiling remains a hard cap that no node can widen.
