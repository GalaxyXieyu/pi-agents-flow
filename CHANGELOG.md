...
- Added Tasks/Agents perspectives switched with `v` (choice persists per session), `↓` activation at an empty editor, `↑↓`/`j`/`k` selection, `x` expansion of active rows only, `Enter` inspector, and `Esc` return. Statuses render as symbols only (`◐ ● ✕ ○ ◌ ⏸ ⊘`); terminal tasks collapse to one row and Agent rows never show model, thinking, or raw tool JSON.
- Removed the legacy above-editor async widget, the `fleetView`/`fleetViewPlacement`/`asyncWidget` config keys, the Workflow Board TUI, and their tests. `/subagents-fleet` remains the deep transcript inspector for steer/stop.

### Fork
- Rebranded the learning fork as `pi-agents-flow` and moved it into `GalaxyXieyu/pi` under `learning/pi-harness/extensions/pi-agents-flow`.
- Renamed the package, parent orchestration skill, runtime state namespace, event protocols, profiles, diagnostics, and tests while retaining the original MIT license and upstream history below.
- Replaced the third-party npm installation path with installation from the checked-out teaching repository.

### Added
- Added `/coding plan|build|verify|full` as one durable deterministic Coding Workflow entry point. Each stage persists a complete WorkflowDataContract V1 DAG over known hidden Agents; `full` requires native user approval of the verified plan before implementation becomes ready.
- Added independent Agent `visibility` (`default` or `hidden`) and `invocation` (`both`, `model`, `user`, or `disabled`) policies, with trusted model/user origin enforcement, native confirmation for model-requested persistent management changes, model-catalog filtering, progressive-disclosure saved chains over hidden model-invocable Agents, user slash routing, settings overrides, management support, and backward-compatible defaults.
- Added configurable Deep Research `WorkflowPolicy` for gate, evidence, and quality thresholds, with optional `policy` overrides on workflow start.
- Added accepted-claim normalization helpers: near-duplicate merge, uncited claim drop, evidence ranking, and tracking-param URL dedupe.
- Added a pure research-lane search quality benchmark and source ranking/follow-up query helpers.
- Added evaluate/status/continuation repair guidance with prioritized next actions and suggested follow-up queries.
- Attached searchBenchmark and recommendedFollowUpQueries onto quality reports when release is blocked.
- Added an experimental supervisor-owned Dynamic Workflow with `/swarm`, `/workflow`, and `/deep-research` commands, durable append-only graph state, typed ephemeral AgentSpecs, parallel ready-node scheduling, structured result envelopes, explicit acceptance/rejection, gap and conflict repair, accepted-claim bundles, citation-safe Writer/Reviewer roles, final Markdown delivery artifacts, settled-idle continuation, waiting-child recovery, and exact stop cancellation.
...
### Changed
- Consolidated the user-level `plan-flow`, `build-flow`, and `verify-flow` saved chains into the `/coding` Workflow preset and removed stale `humanizer` references from Workflow skills.
- Workflow completion and quality release checks now use resolved policy instead of hard-coded Deep Research constants.
- Accepted bundle materialization now applies evidence policy before Writer context is written.
- Reused pi-subagents delegation v2, launch preflight, existing Agent tool allowlists, and normal Pi model/provider configuration for workflow children. Workflow child schemas are strengthened to the canonical WorkflowResult contract before preflight and launch. Workflow state remains the sole source of truth; model-generated workflow code remains outside V0.
- Assigned first-wave Deep Research nodes distinct source portfolios and changed duplicate-source quality scoring to measure cross-lane overlap instead of repeated use inside one lane.
- Required the Supervisor to submit a complete static DAG in one `apply_plan` call when nodes and dependencies are knowable before execution, instead of staging a known plan/implementation/verification chain across separate calls. This keeps execution order enforced by the Scheduler rather than by the Supervisor's running context, which degrades on long flows. The graph may still be extended when a completed result reveals topology that was not knowable at planning time.
- Relaxed the ephemeral-spec capability rule from "never expand" to "never exceed the capability ceiling", which remains monotonically narrowing and is never widened by a launch. Extensions are still never expanded per node.

### Breaking Changes
- Replaced `ResultEnvelopeV0` with `WorkflowResult`: `summary` is now `{text, covers, omissions, confidence}` (bounded, max 8 KiB); `findings`/`search` moved to `evidence`; `gaps`/`conflicts` moved to `diagnostics`; `artifactPaths` removed in favor of named `outputs` port submissions.
- Every work unit must now declare a `dataContract` with `profile`, `inputs`, and `outputs`. The runtime rejects plans without it.
- `resultSchema` removed from `EphemeralAgentSpec`; the runtime generates the structured-output schema from `dataContract.outputs`.
- `complete` no longer accepts `finalMarkdown`; it requires `nodeId`, `port`, and `digest` from an accepted output artifact.
- Full results are no longer written to the event log. Only bounded previews are stored; complete results live in the content-addressed artifact store and are hydrated on load.
- Deep Research handoffs are no longer assembled per-`kind` in the scheduler. All nodes receive a uniform Context Pack generated from `dataContract.inputs`.
- Removed all `V0`/`V1`/`V2` suffixes from type names and constants (`ResearchBrief`, `DocumentOutline`, `EphemeralAgentSpec`, `WorkflowBinding`, `WorkflowClarification*`, `SUBAGENT_DELEGATION_PROTOCOL_VERSION`, `MAX_*` sizes, etc.).

### Added
- Added `WorkflowDataContract` with explicit `inputBindings` (named, from-nodeId+port, delivery mode, merge policy, budget) and `outputPorts` (mediaType, storage, required, classification, schema).
- Added a content-addressed artifact store with SHA-256 integrity, path-escape prevention, preallocated output slots, and atomic writes.
- Added `ContextPack` materialization: resolves only accepted direct dependencies, supports fan-in merge (`list`/`map-by-node`/`concat-text`/`first`), enforces byte/token budgets, and records delivery mode and degradation in a manifest.
- Added classification-based clearance: `public` < `internal` < `sensitive` < `secret`; downstream nodes with insufficient clearance are rejected.
- Added graph-level data flow validation: hidden dependencies, undeclared ports, ambiguous fan-in, and invalid delivery/merge/storage values are rejected at `apply_plan` time.

### Fixed
- Scoped Deep Research editor/reviewer handoffs to accepted direct dependencies, added a no-search default review contract with hard time/turn/tool-call budgets and preflight-visible final quality requirements, made failed/cancelled retries explicit through `run_ready.nodeId`, and allowed accepted same-kind replacements to supersede failed terminal nodes without losing attempt history.
- Preserved the selected Tasks or Agents perspective when opening the Activity Board, rendered Tasks as a left-side plan tree with right-side plan/Agent details, retained terminal workflow plans for review, added duration/token/cost visibility, hid failed Agents behind the `f` toggle, excluded unassigned Work Units from the Agents roster, propagated the resolved workflow language into Fleet so employee identities stay consistent across views, returned from Fleet to the same board selection, kept live Agent selection stable across reordering, and corrected invisible frame lines and oversized empty board layouts.
- Added recorded Evidence/Run file paths to Task, Work Unit, and Agent details, with OSC 8 file hyperlinks and preserved `:line[:column]` locations for terminal/editor navigation.
- Added capability-gated Fleet steering for active foreground single Workflow children, using the same correlated child inbox and acknowledgment protocol as async steering.
- Added a file-reference path for large `structured_output` results: children can submit a bounded schema-valid JSON artifact with an optional SHA-256 digest instead of placing the full result in provider tool arguments. The runtime enforces workspace boundaries, validates before capture, and atomically replaces the canonical output file.
- Preserved one immutable Workflow attempt when a foreground child detaches for supervisor coordination, recovered its result after reload, and removed the temporary waiting reason after successful completion.
- Kept sibling nodes and the workflow active when cancelling one running or waiting node.

### Fixed
- Persisted the Coding workflow contract (`codingContract`) onto `WorkflowRun` via a new `codingStage` start parameter, so `/coding` approval gates and completion-source checks enforce the intended security policy.
- Rejected non-readonly planning repairs before a full Coding plan gate is accepted, while still allowing the complete initial plan to be applied.
- Replaced pending/queued Activity Board/Dock fallback text "Duration unavailable · tokens n/a" with "等待中"/"Pending" and localized Activity Board/Dock labels based on the workflow language.
