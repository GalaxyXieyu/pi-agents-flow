# Pi Agents Flow Workflow Cockpit 交互设计评审稿

> 状态：待用户 Review
>
> 范围：下一轮 pi-agents-flow 的交互入口、看板、视图切换、快捷键和页面间状态同步。
>
...
Pi Agents Flow 的主交互不应该是让用户在输入框里记忆和输入大量 slash 命令。
...
如果宿主键位冲突，应加入 pi-agents-flow 可配置 keymap，不在代码中硬编码。
...
如果这五个问题在每个页面都能被回答，pi-agents-flow 的多页面交互就不会退化成“记命令的控制台”。
